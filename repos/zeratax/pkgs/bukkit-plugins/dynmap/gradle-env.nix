...
{ lib, stdenv, buildEnv, fetchs3, fetchurl, gradleGen, writeText, writeTextDir
}:

{
# Path to the environment spec generated by gradle2nix (e.g. gradle-env.json).
envSpec, pname ? null, version ? null, enableParallelBuilding ? true
...
let
  ...
  mkModuleMetadata = deps:
    let
      ...
      modules = groupBy' (meta: id:
        let
          isNewer = versionOlder meta.latest id.version;
          isNewerRelease = !(hasSuffix "-SNAPSHOT" id.version)
        ...
        in {
          ...
          "${
            replaceStrings [ "." ] [ "/" ] id.group
          }/${id.name}/maven-metadata.xml") ids;

    in attrValues (mapAttrs (path: meta:
      ...
      modules = groupBy' (meta: dep:
        ...
        in {
          ...
          versions = meta.versions or [ ] ++ [{
            ...
            value = "${removeSuffix "-SNAPSHOT" id.version}-${dep.timestamp}-${
                toString dep.build
              }";
            updated = updated;
          }];
          ...
          "${
            replaceStrings [ "." ] [ "/" ] dep.id.group
          }/${dep.id.name}/${dep.id.version}/maven-metadata.xml") snapshotDeps;

      mkSnapshotVersion = version: ''
        <snapshotVersion>
          ${
            optionalString (version.classifier != "")
            "<classifier>${version.classifier}</classifier>"
          }
          <extension>${version.extension}</extension>
          <value>${version.value}</value>
          ${
            optionalString (version.updated != "")
        ...
        <metadata modelVersion="1.1">
          ...
          <versioning>
            <snapshot>
              ...
              ${
                optionalString (buildNumber != -1)
                "<buildNumber>${toString buildNumber}</buildNumber>"
              }
            </snapshot>
            ${
  ...
  mkRepo = project: type: deps:
    buildEnv {
      ...
      paths = map mkDep deps ++ mkModuleMetadata deps
        ++ mkSnapshotMetadata deps;
    };
  ...
  mkInitScript = projectSpec: gradle:
    let
      ...
      hasDependencies =
        mapAttrs (type: deps: deps != [ ]) projectSpec.dependencies;

      inSettings = pred: script:
        optionalString pred (if versionAtLeast gradle.version "6.0" then ''
        ...
        '');
    in assert (assertMsg
      (hasDependencies.settings -> versionAtLeast gradle.version "6.0") ''
  ...
  buildProject = env: flags: ''
    ...
    gradle --offline --no-daemon --no-build-cache \
      ...
      ${
        optionalString (buildJdk != null)
        "-Dorg.gradle.java.home=${buildJdk.home}"
      } \
  ...
  buildIncludedProjects = concatStringsSep "\n"
    (mapAttrsToList (_: env: buildProject env [ "build" ])
