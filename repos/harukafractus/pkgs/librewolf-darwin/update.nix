...
function generate_json_librewolf(){
    ...
    sed -i "/version =/c \ \ version = \"$(echo $base_json_librewolf | jq -r '.[0].tag_name')\";" default.nix;
    sed -i "/$1_url/c \ \ $1_url = \"$url\";" default.nix;
    sed -i "/$1_sha256/c \ \ $1_sha256 = \"$(curl -sL $url.sha256sum)\";" default.nix;
}
