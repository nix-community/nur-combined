...
pkgs.writeShellApplication {
  ...
  text = ''
	...
	function get_github(){
		...
		dl_url=$(curl "https://api.github.com/repos/$repo_owner/$repo_name/releases" |  sd "\"" "\n" | grep -E "https.*\\$match_case$" | head -n 1)
	...
	function get_gitlab(){
		...
		dl_url=$(curl "https://gitlab.com/api/v4/projects/$namespace/releases" |  sd "\"" "\n" | grep -E "https.*\\$match_case$" | head -n 1)
		ver=$(urldecode "$(echo "$dl_url" | sd '/' '\n' | tail -2 | head -1)")
		sha256=$(curl -sL "$dl_url" | sha256sum | awk '{print $1}')
