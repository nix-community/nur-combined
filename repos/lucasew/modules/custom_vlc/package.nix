{ pkgs, lib, stdenv, fetchurl, autoreconfHook
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [
    ...
    ++ optionals withQt5   [ qt.qtbase qt.qtsvg qt.qtx11extras ]
    ++ optionals skins2Support (with pkgs; [ xorg.libXpm freetype xorg.libXext xorg.libXinerama ])
    ++ optional jackSupport libjack2
    ++ optionals chromecastSupport [ protobuf libmicrodns ];

  nativeBuildInputs = [ autoreconfHook perl pkgconfig removeReferencesTo ]
    ++ optionals withQt5 [ qt.wrapQtAppsHook ];
  ...
  '' + optionalString withQt5 ''
    remove-references-to -t "${qt.qtbase.dev}" $out/lib/vlc/plugins/gui/libqt_plugin.so
  '';
