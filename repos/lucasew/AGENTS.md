...
- Never run `modot codebase apply` on this repo.
...
- `modules/` - modot modules (configs, base16 themes)
- `bin/` - CLI scripts (`source bin/source_me` for env); `bin/prelude` is only `sd` shim code — see `bin/prelude/README.md`
- `config/` - modot home templates (source of truth for shell snippets, not `bin/prelude`)
...
- Modot: `modot home apply` / `modot home plan` / `modot driver doctor`
