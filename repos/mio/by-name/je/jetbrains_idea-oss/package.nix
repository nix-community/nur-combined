...
let
  ...
  mkJetBrainsSource = pkgs.callPackage "${pkgs.path}/pkgs/applications/editors/jetbrains/source/build.nix" { 
    jetbrains = jetbrains // { jdk-no-jcef-21 = pkgs.jdk25; };
  };

  newSrc = (mkJetBrainsSource {
    version = newVersion;
    ...
    patches = pkgs.lib.filter (p: !pkgs.lib.hasSuffix "no-download.patch" (builtins.toString p)) old.patches ++ [ ./no-download-2026.2.patch ./jps-compilation-runner.patch ];

    nativeBuildInputs = pkgs.lib.forEach old.nativeBuildInputs (input:
    ...
    configurePhase = builtins.replaceStrings ["ln -s \"$repo\"/.m2 ../.m2"] ["cp -r \"$repo\"/.m2 ../.m2 && chmod -R +w ../.m2\n    sh ${./fix-kotlin-compile.sh} ${pkgs.kotlin}\n    "] old.configurePhase;
