...
/// Ignore sub-pixel jitter when mapping two-finger touch pans to scroll events.
...
impl WaylandClient {
    pub(crate) fn new() -> Self {
                    ...
                    XDPEvent::WindowAppearance(appearance) => {
                        if let Some(client) = client.0.upgrade() {
                            ...
                            let windows = {
                                ...
                                c.common.appearance = appearance;
                                c.windows.values().cloned().collect::<Vec<_>>()
                            };
        ...
        let state = Rc::new(RefCell::new(WaylandClientState {
            ...
            is_scrolling: false,
            touchscreen_drag_scrolls: std::env::var("GPUI_TOUCHSCREEN_DRAG_SCROLLS")
                .map(|v| v != "0")
                .unwrap_or(true),
            touch_window: None,
    ...
    fn event(
        ...
        match event {
            ...
            wl_pointer::Event::Button {
                ...
                if state.mouse_focused_window.is_none() && state.pointer_button_window.is_none() {
                    return;
                }
                match button_state {
        ...
        match event {
            wl_touch::Event::Down {
                ...
                if state.active_touch_id.is_none() {
                    ...
                    // Second finger → two-finger scroll mode.
                    ...
                    if let Some(first_pos) = state.touch_location {
                        let center = point(
                            px((f32::from(first_pos.x) + f32::from(position.x)) / 2.0),
                            px((f32::from(first_pos.y) + f32::from(position.y)) / 2.0),
                        );
