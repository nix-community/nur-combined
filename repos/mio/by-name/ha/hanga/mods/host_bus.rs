const BUS_TOPICS: &str = "ping,name,catalog,hello,voxel,probe,has,methods,gravity,supported-locales,player-spawn,vehicle-spawn-count,vehicle-spawn,vehicle-kit,ambient-agent-count,ambient-agent-spawn,fracture-kit,sound-kit,container-kit,evaluate-action,should-spawn-agent,wallet-after,contract-mark,action-range,loot-item,craft-result,can-complete,crash-kit,steer,fire-kit,tick,should-despawn-agent,story-event,event-label,offer-contract,economy-params,economy-price,voxel-label,contract-label,item-label";
...
fn host_bus_reply(
    ...
    Some(match topic {
        ...
        "container-kit" => crate::container_kit(payload_str(payload, "voxel")),
