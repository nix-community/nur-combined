...
error_output=$(nix-build "$REPO_ROOT" -A aw-watcher-media-player 2>&1 || true)
...
if nix-build "$REPO_ROOT" -A aw-watcher-media-player --no-out-link; then
...
if command -v git &> /dev/null && [ -d "$REPO_ROOT/.git" ]; then
    git add default.nix
    git commit -m "aw-watcher-media-player: $CURRENT_VERSION -> $LATEST_VERSION" || true
fi
