/*
FIXME
ERROR: /build/_bazel_nixbld/96c321f6d4067432c019ff2725c18acb/external/bazel_tools/tools/jdk/BUILD:68:1: every rule of type java_toolchain_alias implicitly depends upon the target '@bazel_tools//tools/jdk:toolchain_hostjava11', but this target could not be found because of: no such target '@bazel_tools//tools/jdk:toolchain_hostjava11': target 'toolchain_hostjava11' not declared in package 'tools/jdk' (did you mean 'toolchain_java11'?) defined by /build/_bazel_nixbld/96c321f6d4067432c019ff2725c18acb/external/bazel_tools/tools/jdk/BUILD
no such target '@bazel_tools//tools/jdk:toolchain_hostjava11':
target 'toolchain_hostjava11' not declared in package 'tools/jdk'
(did you mean 'toolchain_java11'?)
defined by /build/_bazel_nixbld/96c321f6d4067432c019ff2725c18acb/external/bazel_tools/tools/jdk/BUILD
*/
...
let
  # fix: third_party/ijar/mapped_file_unix.cc:115:21: error: 'numeric_limits' is not a member of 'std'
  # FIXME: third_party/ijar/zlib_client.h:64:46: error: 'numeric_limits' is not a member of 'std'

/*
FIXME error: 'numeric_limits' is not a member of 'std'
https://github.com/bazelbuild/bazel/issues?q=error%3A+%27numeric_limits%27+is+not+a+member+of+%27std%27
...
third_party/ijar/mapped_file_unix.cc:115:45: error: '::max' has not been declared; did you mean 'std::max'?
  115 |                std::numeric_limits<size_t>::max());
      |                                             ^~~
      |                                             std::max
In file included from /nix/store/sa3kirarbizwzyz36j8cbac2kgyh28yk-gcc-12.3.0/lib/gcc/x86_64-unknown-linux-gnu/12.3.0/../../../../include/c++/12.3.0/algorithm:61,
                 from third_party/ijar/mapped_file_unix.cc:21:
/nix/store/sa3kirarbizwzyz36j8cbac2kgyh28yk-gcc-12.3.0/lib/gcc/x86_64-unknown-linux-gnu/12.3.0/../../../../include/c++/12.3.0/bits/stl_algo.h:5756:5: note: 'std::max' declared here
 5756 |     max(initializer_list<_Tp> __l, _Compare __comp)
      |     ^~~
*/
  version = "2.1.0";
  # https://github.com/bazelbuild/bazel/releases/tag/2.2.0
  #version = "2.2.0";

/*
FIXME
       error: attribute '"android_tools_pkg-0.13.tar.gz"' missing

       at /home/user/src/milahu/nur-packages/pkgs/development/tools/build-managers/bazel/bazel_2/bazel_2.nix:67:7:

           66|       srcs.build_bazel_rules_nodejs
           67|       srcs."android_tools_pkg-0.13.tar.gz"
             |       ^
           68|       srcs."0.28.3.tar.gz"
       Did you mean android_tools_pkg-0.14.tar.gz?
*/
  ...
  src = fetchurl {
    ...
    #version = "2.2.0";
    #sha256 = "sha256-k3mHioNNEFpHqH09e5gYUt2fZLwWYg6s1WS0hTPhaac=";
  };

  # TODO restore src-deps.json for version 2.1.0
  ...
  srcDepsSet =
    let
      ...
      srcs = (builtins.fromJSON (builtins.readFile ./src-deps.json.2.1.0));
      ...
      srcs."android_tools_pkg-0.13.tar.gz" # 2.1.0
      #srcs."android_tools_pkg-0.14.tar.gz" # 2.2.0
      #srcs."0.28.3.tar.gz" # bazel_toolchains
...
*/
    genericPatches = ''
      ${python311}/bin/2to3 -w -n --no-diffs tools/j2objc/j2objc_header_map.py
      ${python311}/bin/2to3 -w -n --no-diffs tools/j2objc/j2objc_wrapper.py
      ${python311}/bin/2to3 -w -n --no-diffs tools/objc/j2objc_dead_code_pruner.py
    ...
    + genericPatches
    # fix: third_party/ijar/mapped_file_unix.cc:115:21: error: 'numeric_limits' is not a member of 'std'
    # fix: third_party/ijar/zlib_client.h:64:46: error: 'numeric_limits' is not a member of 'std'
    + ''
      sed -i '1i\#include <limits> // std::numeric_limits' third_party/ijar/mapped_file_unix.cc
      sed -i '1i\#include <limits> // std::numeric_limits' third_party/ijar/zlib_client.h
    '';
  ...
  # TODO bazel --config=no-tty
  # https://github.com/pytorch/pytorch/commit/b37578b3c0ddd717435d70ec69a92327b09615d7
  # Make bazel output less verbose. Adds `bazel --config=no-tty` that is useful for less verbose output
/*
...
WARNING: Illegal reflective access by com.google.protobuf.UnsafeUtil (file:/build/bazel_mqFokSxd/archive/libblaze.jar) to field java.nio.Buffer.address
...
Loading: 0 packages loaded
    Fetching @bazel_toolchains; fetching
    Fetching https://mirror.bazel.build/...2.tar.gz; Checking in SHA-256 cache
Loading: 0 packages loaded
    Fetching @bazel_toolchains; fetching
Loading: 0 packages loaded
    Fetching @bazel_toolchains; fetching
    Fetching ...chains; Extracting /build/bazel_mqFokSxd/out/external/bazel_to\
*/
...
*/
  ...
  buildPhase = ''
    sed -i '2i\set -x' bazel_src/compile.sh
    # ERROR: Config value no-tty is not defined in any .rc file
    #sed -i 's/bazel_build/bazel_build --config=no-tty/' bazel_src/compile.sh
    sed -i 's/bazel_build/bazel_build --help || true; bazel_build/' bazel_src/compile.sh
