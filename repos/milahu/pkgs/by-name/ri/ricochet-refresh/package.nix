...
stdenv.mkDerivation (finalAttrs: rec {
  ...
  # build helper to generate pluggables.hpp
  build_pluggables = rustPlatform.buildRustPackage (finalAttrs_libtego: rec {
    pname = "ricochet-refresh-build_pluggables";
    inherit (finalAttrs) version src meta;
    # fix: crates/tego-rs/build.rs: CARGO_TARGET_DIR not set
    CARGO_TARGET_DIR = "target";
    cargoHash = "sha256-tSOmvbm1POi+AUahxePI5YqxQZ/v4Wv2S5flCyOm1zw=";
    sourceRoot = "${finalAttrs_libtego.src.name or finalAttrs.pname}/src/ricochet-refresh-qt";
    doCheck = false;
  });
  ...
  postPatch = ''
    ...
    # use build_pluggables to generate pluggables.hpp
    # fix: error: failed to get `anyhow` as a dependency of package `build_pluggables`
    substituteInPlace libtego_rs/CMakeLists.txt \
      --replace-fail \
      ...
      --replace-fail \
        ...
        '# add_subdirectory(crates)'
    substituteInPlace ricochet-refresh-qt/CMakeLists.txt \
      --replace-fail \
        'COMMAND env CARGO_TARGET_DIR=''${CMAKE_CURRENT_BINARY_DIR} RUST_BACKTRACE=full cargo run --bin build_pluggables' \
        'COMMAND ${build_pluggables}/bin/build_pluggables'
  '';
  ...
  nativeBuildInputs = [
    ...
    # no: error: failed to get `anyhow` as a dependency of package `build_pluggables`
    # cargo
