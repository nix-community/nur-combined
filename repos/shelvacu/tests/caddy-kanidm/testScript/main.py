...
resp = client.succeed(f"curl -u test:test --fail https://{file_server_domain}/awesomefile.txt")
assert resp.strip() == "This is the contents of the awesome file"
resp = client.succeed(f"curl -u test:test2 --fail https://{file_server_domain}/awesomefile.txt")
assert resp.strip() == "This is the contents of the awesome file"
client.fail(f"curl -u test:badpass --fail https://{file_server_domain}/awesomefile.txt")
client.fail(f"curl -u baduser:badpass --fail https://{file_server_domain}/awesomefile.txt")
...
async def run(playwright: Playwright) -> None:
    ...
    await expect(page.get_by_text("Multi-Factor Authentication is required for your account. Delete the generated")).to_be_visible()
    await asyncio.sleep(0.1)
    ...
    await page.get_by_role("textbox", name="Enter a name for your TOTP").fill("otp for test")
    await page.get_by_role("textbox", name="Enter a TOTP code to confirm").fill(totp.now())
    await asyncio.sleep(0.1)
    ...
    await page.get_by_role("textbox", name="Two-factor authentication code").fill(totp.now())
    await page.get_by_role("button", name="Submit").click()
    await page.get_by_role("textbox", name="Password").fill("thisisdefinitelyasecurepassword")
    await page.get_by_role("button", name="Submit").click()
    await page.get_by_role("button", name="Proceed").click()
    await expect(page.locator("pre")).to_contain_text("This is the contents of the awesome file")
