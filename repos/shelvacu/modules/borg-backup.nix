...
in
{
  options.vacu.backup = {
    ...
    repo = mkOption {
      type = types.str;
      default = "${cfg.rsyncUser}@${cfg.rsyncHost}:borg-repos/${cfg.repoPath}";
      defaultText = "(output)";
    };
    package = mkOption {
      type = types.package;
    ...
    keyName = mkOption { type = types.str; };
    keyPath = mkOption {
      type = types.path;
      default = config.sops.secrets.${cfg.keyName}.path;
      defaultText = "(out)";
    };
  };
  config = {
    ...
    systemd.services.auto-borg = {
      script = ''
        ...
        archive_name="vacu-auto-backup--$dashed_date"
        export BORG_PASSPHRASE
        BORG_PASSPHRASE="$(cat ${lib.escapeShellArg cfg.keyPath})"
