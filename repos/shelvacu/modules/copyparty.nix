...
let
    ...
    { name, config, ... }:
    {
      ...
      config = {
        globalConfig = ''
          ...
          }:${config.socketGroup}:${config.socketPath}
  ...
  volumeModule =
    { name, config, ... }:
    {
      imports = [ (lib.mkRenamedOptionModule [ "accs" ] [ "access" ]) ];
      options = {
        enable = mkOption {
          type = types.bool;
          default = true;
  ...
  getBindPaths =
    cfg: readOnly:
    lib.pipe cfg.volumes [
      builtins.attrValues
      (builtins.filter (v: v.bind.enable && (v.bind.readOnly == readOnly)))
      (map (
        v:
        "${systemdEscapePath v.bind.path}:${systemdEscapePath v.bind.path}:${
          if v.bind.rbind then "rbind" else "norbind"
        }"
      ))
    ];
...
in
{
  ...
  config = {
    ...
    systemd.services = mergeWhereEach (cfg: cfg.configureFileServer) (cfg: {
      "copyparty-${cfg.instanceName}" = {
        ...
        serviceConfig = {
          ...
          BindPaths = [ cfg.socketDir ] ++ getBindPaths cfg false;
          BindReadOnlyPaths = [ ] ++ getBindPaths cfg true;
    ...
    vacu.packages = mergeWhereEach (cfg: cfg.configureFileServer) (
      cfg:
      lib.singleton (
