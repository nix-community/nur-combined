...
let
  ...
  eachBackup =
    f:
    lib.mapAttrs' (backupCfg: lib.nameValuePair "borg-backup-${backupCfg.name}" (f backupCfg)) cfg.backups;
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = eachBackup (backupCfg:
    let
