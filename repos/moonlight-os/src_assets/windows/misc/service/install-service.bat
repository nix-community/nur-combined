...
set SERVICE_NAME=HeliosService
set "SERVICE_BIN=%ROOT_DIR%\tools\heliossvc.exe"
set "SERVICE_CONFIG_DIR=%LOCALAPPDATA%\SudoMaker\Helios"
...
rem Check if HeliosService already exists
...
sc %SC_CMD% %SERVICE_NAME% binPath= "\"%SERVICE_BIN%\"" start= %SERVICE_START_TYPE% DisplayName= "Helios Service"

rem Set the description of the service
sc description %SERVICE_NAME% "Helios is a self-hosted game stream host for Moonlight."
