/**
 * @file tools/heliossvc.cpp
 * @brief Handles launching Helios.exe into user sessions as SYSTEM
 */
...
#define SERVICE_NAME "HeliosService"
...
DWORD WINAPI HandlerEx(DWORD dwControl, DWORD dwEventType, LPVOID lpEventData, LPVOID lpContext) {
  switch (dwControl) {
    ...
    case SERVICE_CONTROL_SESSIONCHANGE:
      // If a new session connects to the console, restart Helios
...
HANDLE CreateJobObjectForChildProcess() {
  ...
  // Kill Helios.exe when the final job object handle is closed (which will happen if we terminate unexpectedly).
  // This ensures we don't leave an orphaned Helios.exe running with an inherited handle to our log file.
  job_limit_info.BasicLimitInformation.LimitFlags |= JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE;

  // Allow Helios.exe to use CREATE_BREAKAWAY_FROM_JOB when spawning processes to ensure they can to live beyond
...
HANDLE OpenLogFileHandle() {
  ...
  // Create helios.log in the Temp folder (usually %SYSTEMROOT%\Temp)
  GetTempPathW(_countof(log_file_name), log_file_name);
  wcscat_s(log_file_name, L"helios.log");
...
bool RunTerminationHelper(HANDLE console_token, DWORD pid) {
  ...
  // This will allow us to attach to Helios's console and send it a Ctrl-C event.
...
VOID WINAPI ServiceMain(DWORD dwArgc, LPTSTR *lpszArgv) {
  ...
  // Only allow Helios.exe to inherit the log file handle, not all inheritable handles
  ...
  // Loop every 3 seconds until the stop event is set or Helios.exe is running
  while (WaitForSingleObject(stop_event, 3000) != WAIT_OBJECT_0) {
    ...
    // Start Helios.exe inside our job object
    ...
    PROCESS_INFORMATION process_info;
    if (!CreateProcessAsUserW(console_token, L"Helios.exe", nullptr, nullptr, nullptr, TRUE, CREATE_UNICODE_ENVIRONMENT | CREATE_NO_WINDOW | EXTENDED_STARTUPINFO_PRESENT, nullptr, nullptr, (LPSTARTUPINFOW) &startup_info, &process_info)) {
      CloseHandle(console_token);
      CloseHandle(job_handle);
      continue;
    }
    ...
    bool still_running;
    do {
      // Wait for the stop event to be set, Helios.exe to terminate, or the console session to change
      ...
      switch (WaitForMultipleObjects(_countof(wait_objects), wait_objects, FALSE, INFINITE)) {
        case WAIT_OBJECT_0 + 2:
          if (WTSGetActiveConsoleSessionId() == console_session_id) {
            // The active console session didn't actually change. Let Helios keep running.
            still_running = true;
            continue;
          }
          // Fall-through to terminate Helios.exe and start it again.
        case WAIT_OBJECT_0:
          // The service is shutting down, so try to gracefully terminate Helios.exe.
...
int main(int argc, char *argv[]) {
  ...
  // Check if this is a reinvocation of ourselves to send Ctrl-C to Helios.exe
  ...
  // We want to use the directory where Helios.exe is located instead of system32.
