...
namespace args {
   ...
   * @examples
   * creds("helios", 2, {"new_username", "new_password"});
   * @examples_end
   */
   ...
   * If Helios was improperly terminated, this function restores
   * the global NVIDIA control panel settings to the undo file left
   * by Helios. This function is typically called by the uninstaller.
   * @examples
...
/**
 * @brief Functions for handling the lifetime of Helios.
 */
namespace lifetime {
  extern char **argv;
  ...
  /**
   * @brief Terminates Helios gracefully with the provided exit code.
  ...
  /**
   * @brief Breaks into the debugger or terminates Helios if no debugger is attached.
   */
  void debug_trap();
...
/**
 * @brief Namespace for controlling the Helios service model on Windows.
 */
namespace service_ctrl {
  ...
  /**
   * @brief Wait for the UI to be ready after Helios startup.
   * @examples
   * wait_for_ui_ready();
   * @examples_end
   */
