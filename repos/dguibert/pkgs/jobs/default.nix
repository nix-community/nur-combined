...
with final; let
  jobs = recurseIntoAttrs (lib.virtual (self:
    with self; {
      ...
      sbatchJob = import ./sbatch-job.nix {
        inherit stdenvNoCC lib writeScript runCommand;
      ...
      mkJob = makeOverridable (
        {
          ...
          extraArgs =
            removeAttrs args (["jobImpl" "checks"]
      ...
      mkCheck' = {
        ...
        jobFormalArgs = builtins.functionArgs jobImpl;
        jobArgs = builtins.intersectAttrs jobFormalArgs args;
        check = jobImpl (jobArgs
          // {
            inherit buildInputs;
            /*
            ensure that builtInputs is added
            */
          });
        extraArgs =
          removeAttrs args (["jobImpl"]
            ++ builtins.attrNames jobArgs);
      in
        lib.importJSON (check.out + "/json");
