...
let
  alacrittyHighContrastColors = isDark: {
    primary = {
  ...
  # `ala-${name}` script.
  hmmodules = {
    day = alacrittyMyTheme (alacrittyHighContrastColors false);
    night = alacrittyMyTheme (alacrittyHighContrastColors true);
    naysayer = alacrittyMyTheme (import ./alacritty-naysayer.nix).colors;
  };
  getAlaText =
    hmmodule:
    let
