{ stdenv, callPackage, sources }:

let
  darwinCallPackage = (import sources.nixpkgs-darwin {}).callPackage;
  pkg =
...
in (if stdenv.isDarwin then darwinCallPackage else callPackage) pkg {}
