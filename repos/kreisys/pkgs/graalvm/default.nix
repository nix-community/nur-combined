{ stdenv, lib, fetchFromGitHub, fetchurl, fetchzip, fetchgit, mercurial, python27, setJavaClassPath,
  ...
  # Darwin
  , ApplicationServices, Cocoa, Foundation, JavaVM, Security, CoreFoundation, xcbuild, objc4
}:
...
in rec {
  ...
  jvmci8 = stdenv.mkDerivation rec {
    ...
    buildInputs = [ mx mercurial openjdk ] ++ lib.optionals stdenv.isDarwin [ JavaVM ApplicationServices Cocoa Foundation xcbuild objc4 ];
    postUnpack = ''
    ...
    '';
    patches = [ ./004_mx_jvmci.py.patch ./std-gnu98.patch ];
    ...
    NIX_CFLAGS_COMPILE = 
    (lib.optional stdenv.isLinux "-Wno-error=format-overflow") ++
    [ "-Wno-error=nonnull" "-F${JavaVM}/Library/Frameworks/JavaVM.framework/Frameworks" "-Wno-error=reserved-user-defined-literal" ];
  ...
  graalvm8 = stdenv.mkDerivation rec {
    ...
    meta = with stdenv.lib; {
      ...
      platforms = [ "x86_64-linux" "x86_64-darwin" /*"aarch64-linux" "x86_64-darwin"*/ ];
    };
  };
}
