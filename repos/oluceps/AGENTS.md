...
> This section is for adding new server into this configuration, start from the config: bootstrap

According the file @./mod/hosts/bootstrap , construct a new machine config, you needs to do following:



1. New Node basic info will be in changes of @registry.toml.
2. In mod/disko , create a new file with that hostname with nix suffix, reference to other files and strictly following the disko config that bootstrap.nix file defines.
3. modify mod/age.nix to add new machine vaultix config, add new host config to mod/hosts/* , mod/bird/* , mod/net.nix; reference to other files in same directory, especially named abhoth, since this could be a example of server config.
4. new machine needs to enable yggdrasil and vxlan-mesh (reference to existing config)
