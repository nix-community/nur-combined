...
}:
{
  flake.modules.nixos.honk =
    ...
    {
      imports = [ inputs.dae.nixosModules.honk ];
      vaultix.secrets.dae = {
        owner = "root";
        mode = "400";
      };
      ...
      systemd.services.honk-proxy.serviceConfig = {
        LoadCredential = [
          "config.dae:${pkgs.writeText "config.dae" config.services.honk-proxy.config}"
          "secret.dae:${config.vaultix.secrets.dae.path}"
        ];
        ExecStart = [
          ""
          "${lib.getExe config.services.honk-proxy.package} -c \${CREDENTIALS_DIRECTORY}/config.dae"
        ];
      };
      services.honk-proxy = {
        enable = true;
        disableTxChecksumIpGeneric = false;
        package = inputs.dae.packages.${pkgs.stdenv.hostPlatform.system}.honk-unstable;
        config =
          let
            ...
            experimental {
             ...
             dns {
                 ...
                 upstream {
                     ...
                     googledns: 'tcp+udp://1.1.1.1' -> all
                     pol: 'quic://unfiltered.adguard-dns.com' -> all
                 }
                 routing {
                     request {
