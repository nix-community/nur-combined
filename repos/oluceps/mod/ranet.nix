        ...
        builtins.toJSON [
          {
            nodes =
              ...
              }) (lib.attrNames (lib.filterAttrs (_: v: v.nat) config.data.node)))
              ++ (map (
                i:
                let
              ...
              ) (lib.attrsToList (lib.filterAttrs (_: v: !v.nat) config.data.node)));
            organization = "garden";
...
-----END PUBLIC KEY-----";
    ...
    in
    {
      environment.systemPackages = [
        ...
        inputs.ranet.packages.${pkgs.system}.default
      ];
      ...
      systemd.services.garden =
        let
          cmd = "${
            inputs.ranet.packages.${pkgs.system}.default
          }/bin/ranet --config ${configFile} --registry /var/lib/garden/registry.json --key ${config.vaultix.secrets.garden_key.path}";
        in
        {
