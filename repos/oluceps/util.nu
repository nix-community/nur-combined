...
export def d [
  ...
  if ($nodes == null or $nodes == []) {
    ...
    | each {|i|
        ...
        ssh -t $'ssh://($i.addr)' $'sudo systemd-run -E LOCALE_ARCHIVE --collect --no-ask-password --pipe --quiet --same-dir --service-type=exec --unit=nixos-apply --wait ($i.path)/bin/apply ($mode)' | complete
      }
  }
}
