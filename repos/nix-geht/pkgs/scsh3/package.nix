...
let
  # Native half of the bundled opensc-java.jar
  # Upstream only ships prebuilt x86 binaries.
  ...
  # libltdl, which searches LTDL_LIBRARY_PATH.
  pkcs11Setup = ''
...
stdenv.mkDerivation (finalAttrs: {
  ...
  buildPhase = ''
    ...
    # Build the PKCS#11 JNI library part only.
    $CC -O2 -fPIC -shared \
  ...
  installPhase = ''
    ...
    # arguments against scdp.workspace
  ...
  meta = {
    ...
    longDescription = ''
      ...
      new PKCS11Provider("p11-kit-proxy.so") (or "p11-kit-proxy.dylib" on
      Darwin).

      The OpenCard Framework configuration can be customized per key by
      placing an opencard.properties file in the home directory (as
