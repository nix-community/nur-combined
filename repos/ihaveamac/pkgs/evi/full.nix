...
let
  nixosRuntimepath = writeText "nixos-evirc" ''
    set nocompatible
    syntax on
    ...
    if filereadable("/etc/evirc")
      source /etc/evirc
    elseif filereadable("/etc/evi/evirc")
      source /etc/evi/evirc
    endif
  '';
...
stdenv.mkDerivation {
  ...
  postInstall = ''
    ln -s $out/bin/evi $out/bin/vi
  ''
  + lib.optionalString stdenv.hostPlatform.isLinux ''
    ln -sfn '${nixosRuntimepath}' "$out"/share/evi/evirc
  '';

  postFixup =
    common.postFixup
    + lib.optionalString wrapPythonDrv ''
      wrapProgram "$out/bin/evi" --prefix PATH : "${python3}/bin" \
