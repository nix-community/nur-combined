...
let
  ...
  enaleRootUser = cfg.rootPasswordFile != null;
...

{
  ...
  options = {
    services.gitea = {
      ...
      rootUserName = mkOption {
        ...
        default = if enaleRootUser then "admin" else null;
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.gitea = {
      description = "gitea";
      after = [ "network-online.target" ] ++ optional usePostgresql "postgresql.service" ++ optional useMysql "mysql.service";
      requires = [ "network-online.target" ] ++ optional (cfg.database.createDatabase && usePostgresql) "postgresql.service" ++ optional (cfg.database.createDatabase && useMysql) "mysql.service";
      ...
      in ''
        ...
        ${lib.optionalString enaleRootUser ''
          if [[ $(${exe} admin user list --admin | grep "${cfg.rootUserName}" | wc -l) -eq 0 ]]; then
            ${exe} admin user create --admin \
              --username "${cfg.rootUserName}" \
              --email "${cfg.rootUserEmail}" \
              --password $(<${cfg.rootPasswordFile})
          fi
        ''}
      '';
