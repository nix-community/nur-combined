# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  algermusicplayer-arm = {
    pname = "algermusicplayer-arm";
    version = "5.1.0";
    src = fetchurl {
      url = "https://github.com/algerkong/AlgerMusicPlayer/releases/download/v5.1.0/AlgerMusicPlayer-5.1.0-linux-arm64.AppImage";
      sha256 = "sha256-WaBw7pSstFxqTY6X41iojsJl4KEc5rEWVA3Z5+VqRBc=";
    };
  };
  algermusicplayer-x86 = {
    pname = "algermusicplayer-x86";
    version = "5.1.0";
    src = fetchurl {
      url = "https://github.com/algerkong/AlgerMusicPlayer/releases/download/v5.1.0/AlgerMusicPlayer-5.1.0-linux-x86_64.AppImage";
      sha256 = "sha256-nYMDNSpyyCHRKIBhP++sA7FmodSHBUwRSPEM8yLFaF0=";
    };
  };
  dxvk-gplall = {
    pname = "dxvk-gplall";
    version = "2.7.1-4";
    src = fetchurl {
      url = "https://github.com/Digger1955/dxvk-gplasync-lowlatency/releases/download/DXVK-GPLALL-2.7.1-4/DXVK-GPLALL-MSVC-Windows-AVX2-O1_Oi_Ob3-LTCG-AMD64.2.7.1-4.zip";
      sha256 = "sha256-GB+GHCLSgfv6HQxjEQACzZVK5kUtGKPzBCm+M5z6nIA=";
    };
  };
  enimul = {
    pname = "enimul";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "lzpls";
      repo = "enimul";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-MB36CHrCvtQa5SX/HKw5rYCmPZ4LPXoaNcGUeE/rrAg=";
    };
  };
  fxz = {
    pname = "fxz";
    version = "f12d7eb3ecebcfa5e2d3dc8a016772b3cd101f39";
    src = fetchFromGitHub {
      owner = "conor42";
      repo = "fxz";
      rev = "f12d7eb3ecebcfa5e2d3dc8a016772b3cd101f39";
      fetchSubmodules = false;
      sha256 = "sha256-3NL/3XXmZRv/fYviA2KL/LEx0xVnPk8t4WQt5yxZmA8=";
    };
    date = "2022-01-09";
  };
  jaq = {
    pname = "jaq";
    version = "v3.0.0";
    src = fetchFromGitHub {
      owner = "01mf02";
      repo = "jaq";
      rev = "v3.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-FhmnJm/MV4WfvhF7DWW6t2D876SIzzq7ODlY2Wmo9mM=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-FhmnJm_MV4WfvhF7DWW6t2D876SIzzq7ODlY2Wmo9mM=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  jj-lsp = {
    pname = "jj-lsp";
    version = "0.1.0";
    src = fetchFromGitHub {
      owner = "nilskch";
      repo = "jj-lsp";
      rev = "0.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-Wj809KL9BCv3jDGBDTDF4isTPO68GOncNvflaqMMpBI=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Wj809KL9BCv3jDGBDTDF4isTPO68GOncNvflaqMMpBI=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  kikoflu = {
    pname = "kikoflu";
    version = "v3.1.1";
    src = fetchFromGitHub {
      owner = "pa-jesusf";
      repo = "KikoFlu";
      rev = "v3.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-I2i3NYv2TzqKvkDG8MFFefDcp8E4blxJ0M05wDKSj5Y=";
    };
    extract = {
      "pubspec.lock" = ./. + "/sha256-I2i3NYv2TzqKvkDG8MFFefDcp8E4blxJ0M05wDKSj5Y=/pubspec.lock";
    };
  };
  krunner-fd-plugin = {
    pname = "krunner-fd-plugin";
    version = "2aa89312b2979d552336a5b56e3710b7e8cf48e2";
    src = fetchFromGitHub {
      owner = "wangzk";
      repo = "krunner-fd-plugin";
      rev = "2aa89312b2979d552336a5b56e3710b7e8cf48e2";
      fetchSubmodules = false;
      sha256 = "sha256-nz6zp/e6A8OyRgjps0JA29t7K+DJY+nGeAuY09I/wMU=";
    };
    date = "2026-03-21";
  };
  krunner-zed = {
    pname = "krunner-zed";
    version = "v0.1.5";
    src = fetchFromGitHub {
      owner = "hron";
      repo = "krunner-zed";
      rev = "v0.1.5";
      fetchSubmodules = false;
      sha256 = "sha256-Oaeo6SjMmB39jWMWJAK8aT20HrJ/7vo1rNVhW1Z6/qs=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Oaeo6SjMmB39jWMWJAK8aT20HrJ_7vo1rNVhW1Z6_qs=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  linux-enable-ir-emitter = {
    pname = "linux-enable-ir-emitter";
    version = "7.0.0-beta";
    src = fetchFromGitHub {
      owner = "EmixamPP";
      repo = "linux-enable-ir-emitter";
      rev = "7.0.0-beta";
      fetchSubmodules = false;
      sha256 = "sha256-lHNo00xhyk3objKR1RUPofSxImJ12V1yShAibjKZ7lw=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-lHNo00xhyk3objKR1RUPofSxImJ12V1yShAibjKZ7lw=/Cargo.lock";
      outputHashes = {
        "ansipix-2.0.0" = "sha256-RkwicughatQcuXnPIRtnL36xkfFzCsHDgL+VZCi46WY=";
      };
    };
  };
  loveiwara = {
    pname = "loveiwara";
    version = "0.4.4";
    src = fetchFromGitHub {
      owner = "FoxSensei001";
      repo = "LoveIwara";
      rev = "0.4.4";
      fetchSubmodules = false;
      sha256 = "sha256-Vdf3ERybsOXG8SlNmQ/QRtBu7RS0T9t0V22UTMjXTls=";
    };
    extract = {
      "pubspec.lock" = ./. + "/sha256-Vdf3ERybsOXG8SlNmQ_QRtBu7RS0T9t0V22UTMjXTls=/pubspec.lock";
    };
  };
  lxgw-wenkai-gb = {
    pname = "lxgw-wenkai-gb";
    version = "v1.522";
    src = fetchurl {
      url = "https://github.com/lxgw/LxgwWenkaiGB/releases/download/v1.522/lxgw-wenkai-gb-v1.522.tar.gz";
      sha256 = "sha256-arcHfwuy5GK3FwikzJi7Qo0Cn/+6ShxLCoHin8RTpww=";
    };
  };
  lyrica = {
    pname = "lyrica";
    version = "4fab122046a66e111cfddf5980e307e361139e26";
    src = fetchFromGitHub {
      owner = "chiyuki0325";
      repo = "lyrica";
      rev = "4fab122046a66e111cfddf5980e307e361139e26";
      fetchSubmodules = false;
      sha256 = "sha256-1CJWqbOGND00+xziSnaZVWtvnfhV9epKd7GVbAOQZvw=";
    };
    extract = {
      "frontend/kde/metadata.json" = ./. + "/sha256-1CJWqbOGND00+xziSnaZVWtvnfhV9epKd7GVbAOQZvw=/frontend/kde/metadata.json";
      "Cargo.toml" = ./. + "/sha256-1CJWqbOGND00+xziSnaZVWtvnfhV9epKd7GVbAOQZvw=/Cargo.toml";
    };cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-1CJWqbOGND00+xziSnaZVWtvnfhV9epKd7GVbAOQZvw=/Cargo.lock";
      outputHashes = {
        "netease-cloud-music-api-1.5.2" = "sha256-6YgH9HhgBX+1tT4ysXtqtLENSD2wwhyhcJrB5q/5LTA=";
      };
    };
    date = "2026-05-15";
  };
  motrix-next-beta = {
    pname = "motrix-next-beta";
    version = "v3.9.4-beta.2";
    src = fetchFromGitHub {
      owner = "AnInsomniacy";
      repo = "motrix-next";
      rev = "v3.9.4-beta.2";
      fetchSubmodules = false;
      sha256 = "sha256-JZzZj8f3b4favxRbbX05Qyu4qcpMe9IryyDQmV6ZVZ0=";
    };
    cargoLock."src-tauri/Cargo.lock" = {
      lockFile = ./. + "/sha256-JZzZj8f3b4favxRbbX05Qyu4qcpMe9IryyDQmV6ZVZ0=/src-tauri/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  ntfsprogs-plus = {
    pname = "ntfsprogs-plus";
    version = "53943daef955cc2a4a405c8e37c62478e4272fec";
    src = fetchFromGitHub {
      owner = "ntfsprogs-plus";
      repo = "ntfsprogs-plus";
      rev = "53943daef955cc2a4a405c8e37c62478e4272fec";
      fetchSubmodules = false;
      sha256 = "sha256-IYo8J5bxzDYvJ1sF0nG2F6r+oDHsep0oLroNKH77JrQ=";
    };
    date = "2026-05-07";
  };
  piliplus = {
    pname = "piliplus";
    version = "2.0.8";
    src = fetchFromGitHub {
      owner = "bggRGjQaUbCoE";
      repo = "PiliPlus";
      rev = "2.0.8";
      fetchSubmodules = false;
      sha256 = "sha256-cCMz6VPim6Nhvi3yNYkguhuNdLOqK/W1M8GroHVEhNM=";
    };
    extract = {
      "pubspec.lock" = ./. + "/sha256-cCMz6VPim6Nhvi3yNYkguhuNdLOqK_W1M8GroHVEhNM=/pubspec.lock";
    };
  };
  pixes = {
    pname = "pixes";
    version = "v1.2.1";
    src = fetchFromGitHub {
      owner = "wgh136";
      repo = "pixes";
      rev = "v1.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-eSAIDgKJ0Yg1F3S5Y1ljqlAEwFDuGt/OJn6ia6CmIc4=";
    };
    extract = {
      "pubspec.lock" = ./. + "/sha256-eSAIDgKJ0Yg1F3S5Y1ljqlAEwFDuGt_OJn6ia6CmIc4=/pubspec.lock";
    };
  };
  pixes-git = {
    pname = "pixes-git";
    version = "abf249b941dd5e4e9bd03ac2add8573bd669917b";
    src = fetchFromGitHub {
      owner = "wgh136";
      repo = "pixes";
      rev = "abf249b941dd5e4e9bd03ac2add8573bd669917b";
      fetchSubmodules = false;
      sha256 = "sha256-eSAIDgKJ0Yg1F3S5Y1ljqlAEwFDuGt/OJn6ia6CmIc4=";
    };
    extract = {
      "pubspec.lock" = ./. + "/sha256-eSAIDgKJ0Yg1F3S5Y1ljqlAEwFDuGt_OJn6ia6CmIc4=/pubspec.lock";
    };date = "2026-04-07";
  };
  pwasio = {
    pname = "pwasio";
    version = "7373d56a378807191cc5f9b15fff9b456e7ab8ce";
    src = fetchFromGitHub {
      owner = "golfiros";
      repo = "pwasio";
      rev = "7373d56a378807191cc5f9b15fff9b456e7ab8ce";
      fetchSubmodules = false;
      sha256 = "sha256-XmYf5gbYet7sJxtOfRyDofSKVmaL1XD6LeDInugyUCE=";
    };
    date = "2026-05-23";
  };
  splayer-git = {
    pname = "splayer-git";
    version = "28b76bad9eb800e0c90dac8475d4769acfe2450b";
    src = fetchFromGitHub {
      owner = "imsyy";
      repo = "SPlayer";
      rev = "28b76bad9eb800e0c90dac8475d4769acfe2450b";
      fetchSubmodules = false;
      sha256 = "sha256-7oLFJqZ1Apq2GK5G3r10I+c3liSweDD2ZPhjpq0f+bM=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-7oLFJqZ1Apq2GK5G3r10I+c3liSweDD2ZPhjpq0f+bM=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-06-05";
  };
  splayer-kde-bar-lyc = {
    pname = "splayer-kde-bar-lyc";
    version = "d89404376a2b27dbb58046ce6da894c1e83c9169";
    src = fetchFromGitHub {
      owner = "lrst6963";
      repo = "Splayer-kde-bar-lyc";
      rev = "d89404376a2b27dbb58046ce6da894c1e83c9169";
      fetchSubmodules = false;
      sha256 = "sha256-i/ZA2yMrBmexefwHa0xv9E8CwC4jRJYCwCe6Sycpih4=";
    };
    extract = {
      "metadata.json" = ./. + "/sha256-i_ZA2yMrBmexefwHa0xv9E8CwC4jRJYCwCe6Sycpih4=/metadata.json";
    };date = "2026-02-19";
  };
  splayer-next-dev = {
    pname = "splayer-next-dev";
    version = "035c76a896c886ca002f66696c74614d11b81d36";
    src = fetchFromGitHub {
      owner = "SPlayer-Dev";
      repo = "SPlayer-Next";
      rev = "035c76a896c886ca002f66696c74614d11b81d36";
      fetchSubmodules = false;
      sha256 = "sha256-CEQZ3AJgYNvSxpP+hX1jmFu4Qe04m8WSeeiP9pFvHkM=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-CEQZ3AJgYNvSxpP+hX1jmFu4Qe04m8WSeeiP9pFvHkM=/Cargo.lock";
      outputHashes = {
        "ffmpeg_audio-0.1.0" = "sha256-EA7x3EGb2FzLqp3GklgeP9ZSalyPm7ohZDPrqbSRX0I=";
      };
    };
    date = "2026-06-04";
  };
  svt-av1-essential = {
    pname = "svt-av1-essential";
    version = "v4.0.1-Essential";
    src = fetchFromGitHub {
      owner = "nekotrix";
      repo = "SVT-AV1-Essential";
      rev = "v4.0.1-Essential";
      fetchSubmodules = false;
      sha256 = "sha256-Xyvlirf0lGocQL5xYeWOVKYpDRXNs3O70AotTho/FTE=";
    };
  };
  svt-av1-hdr = {
    pname = "svt-av1-hdr";
    version = "v4.1.0";
    src = fetchFromGitHub {
      owner = "juliobbv-p";
      repo = "svt-av1-hdr";
      rev = "v4.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-n66RPDTfBsPCR/4y8dpU3Au1WZHpkpln899e2+LKxto=";
    };
  };
  svt-av1-psyex = {
    pname = "svt-av1-psyex";
    version = "v3.0.2-B";
    src = fetchFromGitHub {
      owner = "BlueSwordM";
      repo = "svt-av1-psyex";
      rev = "v3.0.2-B";
      fetchSubmodules = false;
      sha256 = "sha256-klfrbow8UtpIPwIgt8tK7FP7Jp6In9nxfOZrdi1PsHo=";
    };
  };
  uosc-danmaku = {
    pname = "uosc-danmaku";
    version = "v2.1.0";
    src = fetchFromGitHub {
      owner = "Tony15246";
      repo = "uosc_danmaku";
      rev = "v2.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-07J+kNj8wkoLn0bWbER1/xoiT1+60sAziKGivy1/X04=";
    };
  };
  uosc-danmaku-git = {
    pname = "uosc-danmaku-git";
    version = "ee8bbb9df9db29a2bd85effbbdd774f80a3d1f26";
    src = fetchFromGitHub {
      owner = "Tony15246";
      repo = "uosc_danmaku";
      rev = "ee8bbb9df9db29a2bd85effbbdd774f80a3d1f26";
      fetchSubmodules = false;
      sha256 = "sha256-+oDyO8EcPY44yiTQ2wmuajD7vHPMphloVPEi93Xaniw=";
    };
    date = "2026-06-03";
  };
  waywallen-bin = {
    pname = "waywallen-bin";
    version = "0.1.8";
    src = fetchurl {
      url = "https://github.com/waywallen/waywallen/releases/download/v0.1.8/waywallen-0.1.8-x86_64.AppImage";
      sha256 = "sha256-OLEJyXTDyWzyyscU8GpwVRlEzhM3zI52ZZL53Unfoew=";
    };
  };
  waywallen-display-bin = {
    pname = "waywallen-display-bin";
    version = "0.2.4";
    src = fetchurl {
      url = "https://github.com/waywallen/waywallen-display/releases/download/v0.2.4/waywallen-kde-0.2.4-x86_64-embed.zip";
      sha256 = "sha256-UzaR09n3EpK64uu5ISYaApct1dB6n9uU80djw8c7kPM=";
    };
  };
  wild = {
    pname = "wild";
    version = "v0.0.14";
    src = fetchFromGitHub {
      owner = "niuhuan";
      repo = "wild";
      rev = "v0.0.14";
      fetchSubmodules = false;
      sha256 = "sha256-Bt2ZjivE0htq730N8HkjnjCMD1lsI5PHtCeIWGxZ9CQ=";
    };
    extract = {
      "pubspec.lock" = ./. + "/sha256-Bt2ZjivE0htq730N8HkjnjCMD1lsI5PHtCeIWGxZ9CQ=/pubspec.lock";
    };cargoLock."rust/Cargo.lock" = {
      lockFile = ./. + "/sha256-Bt2ZjivE0htq730N8HkjnjCMD1lsI5PHtCeIWGxZ9CQ=/rust/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  zhuque = {
    pname = "zhuque";
    version = "v0.212";
    src = fetchurl {
      url = "https://github.com/TrionesType/zhuque/releases/download/v0.212/ZhuqueFangsong-v0.212.zip";
      sha256 = "sha256-u4tmGnZD0ilqctnRBTCgCUlBnE5Sf7YXg/c8K6GowGI=";
    };
  };
}
