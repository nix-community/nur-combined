...
echo "Motrix Next deps build output is:"
echo "$build_output"
hash_motrix_next=$(tr -s ' ' <<<$build_output | grep -Po "got: \K.+$")
if [ -z "$hash_motrix_next" ]; then
  echo "Failed to extract hash from build output."
  exit 1
fi
echo "Motrix Next deps hash is: $hash_motrix_next"

jq -n \
  --arg version "$version" \
  --arg hash "$hash_motrix_next" \
