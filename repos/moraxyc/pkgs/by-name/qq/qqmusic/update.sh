...
update_platform() {
    ...
    local NEW_HASH
    local SRI_HASH
    NEW_HASH=$(nix-prefetch-url --type sha256 "$URL" --name "$FILENAME")
    SRI_HASH=$(nix --extra-experimental-features nix-command hash convert --to sri --hash-algo sha256 "$NEW_HASH")
