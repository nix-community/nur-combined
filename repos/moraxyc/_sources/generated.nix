# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  bark-server = {
    pname = "bark-server";
    version = "e0297015409dfb2a9837fc5f99f6d8ff96f5b36a";
    src = fetchFromGitHub {
      owner = "Finb";
      repo = "bark-server";
      rev = "e0297015409dfb2a9837fc5f99f6d8ff96f5b36a";
      fetchSubmodules = false;
      sha256 = "sha256-Ql1ZkFIsj8vuMWgk6xvaKjTJRoLHe9gjtShuRwnXZys=";
    };
    date = "2025-12-22";
  };
  bark-server-stable = {
    pname = "bark-server-stable";
    version = "v2.3.2";
    src = fetchFromGitHub {
      owner = "Finb";
      repo = "bark-server";
      rev = "v2.3.2";
      fetchSubmodules = false;
      sha256 = "sha256-Ql1ZkFIsj8vuMWgk6xvaKjTJRoLHe9gjtShuRwnXZys=";
    };
  };
  english_words = {
    pname = "english_words";
    version = "20f5cc9b3f0ccc8ce45d814c532b7c2031bba31c";
    src = fetchFromGitHub {
      owner = "dwyl";
      repo = "english-words";
      rev = "20f5cc9b3f0ccc8ce45d814c532b7c2031bba31c";
      fetchSubmodules = false;
      sha256 = "sha256-PfBQ9iavOLx7M8+j0TXASUxVamPiQn2YXsiuUmz4XCg=";
    };
    date = "2025-01-06";
  };
  exloli-next = {
    pname = "exloli-next";
    version = "41d7a6e52a1625d6febb47160a4b143ca47c16d5";
    src = fetchFromGitHub {
      owner = "lolishinshi";
      repo = "exloli-next";
      rev = "41d7a6e52a1625d6febb47160a4b143ca47c16d5";
      fetchSubmodules = false;
      sha256 = "sha256-S3hjuRSDtPkK9XF2USXfyOawWoD/fu9qYDJbUPI3Op0=";
    };
    date = "2024-11-23";
  };
  fcitx5-themes-candlelight = {
    pname = "fcitx5-themes-candlelight";
    version = "653677b0454569f41c815b3d262a57e42c90ee05";
    src = fetchFromGitHub {
      owner = "thep0y";
      repo = "fcitx5-themes-candlelight";
      rev = "653677b0454569f41c815b3d262a57e42c90ee05";
      fetchSubmodules = false;
      sha256 = "sha256-dN77aUt1qkN177BZOfrT6O72Qp0J2jlM2mGNxI0cBnA=";
    };
    date = "2025-11-07";
  };
  flapalerted = {
    pname = "flapalerted";
    version = "36647a2116f8eb6523439ca497f92b7c983d216a";
    src = fetchFromGitHub {
      owner = "Kioubit";
      repo = "FlapAlerted";
      rev = "36647a2116f8eb6523439ca497f92b7c983d216a";
      fetchSubmodules = false;
      sha256 = "sha256-ThWbymqBUiWSE5Q5bQUA5mq4X6Dwf9bg1a+aCaty/ZY=";
    };
    date = "2025-12-27";
  };
  flapalerted-stable = {
    pname = "flapalerted-stable";
    version = "v4.3.0";
    src = fetchFromGitHub {
      owner = "Kioubit";
      repo = "FlapAlerted";
      rev = "v4.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-xoskkM5xdU3+T5AtgGv0uPZEUOLtMGl7RPv8gKBJBho=";
    };
  };
  geolite2-asn = {
    pname = "geolite2-asn";
    version = "2025.12.28";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2025.12.28/GeoLite2-ASN.mmdb";
      sha256 = "sha256-vYyPNt5TNKT/wWschhzdND7Vv3mveENSoHHFhakYkgY=";
    };
  };
  geolite2-city = {
    pname = "geolite2-city";
    version = "2025.12.28";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2025.12.28/GeoLite2-City.mmdb";
      sha256 = "sha256-N7zjx8bZzatSKgGkQV3YsrZ6BWn+sM5Z75q1b6W/Ggw=";
    };
  };
  geolite2-country = {
    pname = "geolite2-country";
    version = "2025.12.28";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2025.12.28/GeoLite2-Country.mmdb";
      sha256 = "sha256-RLn5BT2xhXkIZmngtyxFjQAyToRdVe0QruPltKoQzlc=";
    };
  };
  gzctf = {
    pname = "gzctf";
    version = "v1.8.0";
    src = fetchFromGitHub {
      owner = "GZTimeWalker";
      repo = "GZCTF";
      rev = "v1.8.0";
      fetchSubmodules = false;
      sha256 = "sha256-Z9F8QS+HMaUD9n0YOp2w8KR43iUtzHAV8sMcZO3VPkw=";
    };
  };
  gzctf-contributors = {
    pname = "gzctf-contributors";
    version = "0.0.0";
    src = fetchurl {
      url = "https://api.github.com/repos/GZTimeWalker/GZCTF/contributors";
      sha256 = "sha256-t90DgDDp0Eub8OZaDutZ0PknxSdu5SQPo2fSTUo0rBA=";
    };
  };
  libinput-three-finger-drag = {
    pname = "libinput-three-finger-drag";
    version = "6acd3f84b551b855b5f21b08db55e95dae3305c5";
    src = fetchFromGitHub {
      owner = "marsqing";
      repo = "libinput-three-finger-drag";
      rev = "6acd3f84b551b855b5f21b08db55e95dae3305c5";
      fetchSubmodules = false;
      sha256 = "sha256-xmcTb+23d6mMzIfMVjzN6bwV0fWH4p6YhXXqrFmt4TM=";
    };
    date = "2024-06-17";
  };
  mautrix-telegramgo = {
    pname = "mautrix-telegramgo";
    version = "3696c9cff494e8714b1af6c7c6884c30fe8439c2";
    src = fetchFromGitHub {
      owner = "mautrix";
      repo = "telegramgo";
      rev = "3696c9cff494e8714b1af6c7c6884c30fe8439c2";
      fetchSubmodules = false;
      sha256 = "sha256-KdtilYzvTVbrozgERMJXToOfbxXR9afuaSJEJHISG+M=";
    };
    date = "2025-12-24";
  };
  mm-geofeed-verifier = {
    pname = "mm-geofeed-verifier";
    version = "v3.1.0";
    src = fetchFromGitHub {
      owner = "maxmind";
      repo = "mm-geofeed-verifier";
      rev = "v3.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-14TztORKynlHS277k+W7FN1kgQD0FFu5W07wxgCDFS0=";
    };
  };
  pixy2 = {
    pname = "pixy2";
    version = "d7c38d49ae8071b58e46f5b35eda927dd2b7c621";
    src = fetchFromGitHub {
      owner = "charmedlabs";
      repo = "pixy2";
      rev = "d7c38d49ae8071b58e46f5b35eda927dd2b7c621";
      fetchSubmodules = false;
      sha256 = "sha256-w0t7KqV8OS3PJBdZtLn86zknQYo70rBv7fuK/UFx9zQ=";
    };
    date = "2024-07-17";
  };
  v2ray-rules-dat-geoip = {
    pname = "v2ray-rules-dat-geoip";
    version = "202512272214";
    src = fetchurl {
      url = "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/202512272214/geoip.dat";
      sha256 = "sha256-+clDiSHGhzYtrZQwlEEMkeunav0B++IknnNuyMNRTww=";
    };
  };
  v2ray-rules-dat-geosite = {
    pname = "v2ray-rules-dat-geosite";
    version = "202512272214";
    src = fetchurl {
      url = "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/202512272214/geosite.dat";
      sha256 = "sha256-uMb0Trm+S3cqo0n3DIW/Ywb9ME78ys1scZ+qn0d4h/c=";
    };
  };
  watt-toolkit-aarch64-linux = {
    pname = "watt-toolkit-aarch64-linux";
    version = "3.0.0-rc.16";
    src = fetchurl {
      url = "https://github.com/BeyondDimension/SteamTools/releases/download/3.0.0-rc.16/Steam++_v3.0.0-rc.16_linux_arm64.tgz";
      sha256 = "sha256-xfzxxxMKsoMww9PzB/nvRcxqOqyn65escvxkzK9csCo=";
    };
  };
  watt-toolkit-x86_64-linux = {
    pname = "watt-toolkit-x86_64-linux";
    version = "3.0.0-rc.16";
    src = fetchurl {
      url = "https://github.com/BeyondDimension/SteamTools/releases/download/3.0.0-rc.16/Steam++_v3.0.0-rc.16_linux_x64.tgz";
      sha256 = "sha256-mUk/gh8GPYwhs5OZzKorkNIMMKl+amFrwbTtH0AnHTI=";
    };
  };
}
