# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  aliyundrive-webdav = {
    pname = "aliyundrive-webdav";
    version = "00caff62b55ae53327f1c8824b19b1c9532cc174";
    src = fetchFromGitHub {
      owner = "messense";
      repo = "aliyundrive-webdav";
      rev = "00caff62b55ae53327f1c8824b19b1c9532cc174";
      fetchSubmodules = false;
      sha256 = "sha256-Glr/PryfyihEMM+41ZkXp3I0VeXerHMKO82znLJJ2mc=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Glr_PryfyihEMM+41ZkXp3I0VeXerHMKO82znLJJ2mc=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-02-20";
  };
  aliyundrive-webdav-stable = {
    pname = "aliyundrive-webdav-stable";
    version = "v2.3.3";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  bark-server = {
    pname = "bark-server";
    version = "3df8990fcbc407a3f5638eea8cedc3289d1a405d";
    src = fetchFromGitHub {
      owner = "Finb";
      repo = "bark-server";
      rev = "3df8990fcbc407a3f5638eea8cedc3289d1a405d";
      fetchSubmodules = false;
      sha256 = "sha256-REYmiq4yv6zTHV1x6xmdZUgpQItJRCexnxjoSrpG0tI=";
    };
    date = "2026-07-07";
  };
  bark-server-stable = {
    pname = "bark-server-stable";
    version = "v2.3.5";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  boringssl-oqs = {
    pname = "boringssl-oqs";
    version = "OQS-BoringSSL-snapshot-2025-01";
    src = fetchFromGitHub {
      owner = "open-quantum-safe";
      repo = "boringssl";
      rev = "OQS-BoringSSL-snapshot-2025-01";
      fetchSubmodules = false;
      sha256 = "sha256-zVZgAvq6V85hxO79Ct8s+VVyf9yPa/YbRL4D5z31qEs=";
    };
  };
  clash-nyanpasu = {
    pname = "clash-nyanpasu";
    version = "bc1c1d07e3b65ea4d2196f0590c85f4d889d0326";
    src = fetchFromGitHub {
      owner = "libnyanpasu";
      repo = "clash-nyanpasu";
      rev = "bc1c1d07e3b65ea4d2196f0590c85f4d889d0326";
      fetchSubmodules = false;
      sha256 = "sha256-WTCZhdEuNTBK/JY+zR6i80ODf9vOuA1QbZJMmGKq2cQ=";
    };
    cargoLock."backend/Cargo.lock" = {
      lockFile = ./. + "/sha256-WTCZhdEuNTBK_JY+zR6i80ODf9vOuA1QbZJMmGKq2cQ=/backend/Cargo.lock";
      outputHashes = {
        "delay_timer-0.11.6" = "sha256-Cf2u71+12mQ227Q382cF5WkK9E8wxrJnWDcTq2jX/Gk=";
        "auto-launch-0.5.0" = "sha256-+Pqd9cgp6alJdoBZFX5/35cc77Ay4RRQUE3Xhnm4Sfs=";
        "runas-1.2.0" = "sha256-cTYTFtmdmCztL2JhgnOxwKhd+VfUmm5DB3gkNy92g04=";
        "tracing-test-0.2.5" = "sha256-uhjFW7s8Lfvq/j8mTCXcivzIvwHP3NFoI8rJOZ9avdE=";
        "nyanpasu-ipc-1.4.5" = "sha256-qYFy2VoyPxFwviQUoXGwDVlNWALJE7Yom3EKd/r2EiE=";
        "nyanpasu-utils-0.1.0" = "sha256-02NVeFQWazXTix7g7EGuYZ/l+idpMvbDxiBi4ysZclc=";
        "include_url_macro-0.1.0" = "sha256-uWrm6/8aH2cny172IWwOsPbFW8WMQ1OzO3M0S8DqKX8=";
        "serde_yaml_ng-0.10.0" = "sha256-0KkwfAWwhCELkobRjnhzyQPzcqDdnwm8oAzfL1GjKnk=";
        "sysproxy-0.3.0" = "sha256-2xv1vjthD3J/SFhbnh21bi2V4fcvoo6CpjJDxpSbwnk=";
        "include-compress-bytes-0.1.0" = "sha256-wl02rEb2OCTt8Yv+nwu4AChLC3gN6R53dNtgvM4+q7g=";
      };
    };
    date = "2026-08-15";
  };
  clash-nyanpasu-stable = {
    pname = "clash-nyanpasu-stable";
    version = "v1.6.1";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  cpa-usage-keeper = {
    pname = "cpa-usage-keeper";
    version = "498abd16809e6410e1c544636758a6f21c1e7360";
    src = fetchFromGitHub {
      owner = "Willxup";
      repo = "cpa-usage-keeper";
      rev = "498abd16809e6410e1c544636758a6f21c1e7360";
      fetchSubmodules = false;
      sha256 = "sha256-jCzf9K/b7eEPse5xXhd3Btrday1NtJkpeh2M7cQsbRI=";
    };
    extract = {
      "web/package.json" = ./. + "/sha256-jCzf9K_b7eEPse5xXhd3Btrday1NtJkpeh2M7cQsbRI=/web/package.json";
      "web/package-lock.json" = ./. + "/sha256-jCzf9K_b7eEPse5xXhd3Btrday1NtJkpeh2M7cQsbRI=/web/package-lock.json";
    };date = "2026-08-14";
  };
  cpa-usage-keeper-stable = {
    pname = "cpa-usage-keeper-stable";
    version = "v1.14.4";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  dae = {
    pname = "dae";
    version = "caa6f5e91776bc86d5b0edc940bb7d264359863c";
    src = fetchFromGitHub {
      owner = "daeuniverse";
      repo = "dae";
      rev = "caa6f5e91776bc86d5b0edc940bb7d264359863c";
      fetchSubmodules = true;
      sha256 = "sha256-A+62QTun9gsHL1fJAxbCqGRE8vJJMaI/XO8tPhe0Yw4=";
    };
    date = "2026-07-31";
  };
  dae-stable = {
    pname = "dae-stable";
    version = "v2.0.0";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  elio = {
    pname = "elio";
    version = "v1.11.2";
    src = fetchFromGitHub {
      owner = "elio-fm";
      repo = "elio";
      rev = "v1.11.2";
      fetchSubmodules = false;
      sha256 = "sha256-k+88cWiHi1a+f6oulb5MTCnWrJU4vKPEAHBwq5H9bkQ=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-k+88cWiHi1a+f6oulb5MTCnWrJU4vKPEAHBwq5H9bkQ=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  english_words = {
    pname = "english_words";
    version = "20f5cc9b3f0ccc8ce45d814c532b7c2031bba31c";
    src = fetchFromGitHub {
      owner = "dwyl";
      repo = "english-words";
      rev = "20f5cc9b3f0ccc8ce45d814c532b7c2031bba31c";
      fetchSubmodules = false;
      sha256 = "sha256-PfBQ9iavOLx7M8+j0TXASUxVamPiQn2YXsiuUmz4XCg=";
    };
    date = "2025-01-06";
  };
  exloli-next = {
    pname = "exloli-next";
    version = "41d7a6e52a1625d6febb47160a4b143ca47c16d5";
    src = fetchFromGitHub {
      owner = "lolishinshi";
      repo = "exloli-next";
      rev = "41d7a6e52a1625d6febb47160a4b143ca47c16d5";
      fetchSubmodules = false;
      sha256 = "sha256-S3hjuRSDtPkK9XF2USXfyOawWoD/fu9qYDJbUPI3Op0=";
    };
    date = "2024-11-23";
  };
  fcitx5-themes-candlelight = {
    pname = "fcitx5-themes-candlelight";
    version = "60aeaddfb3ecdb6a132e8da7569a6c442e6bb217";
    src = fetchFromGitHub {
      owner = "thep0y";
      repo = "fcitx5-themes-candlelight";
      rev = "60aeaddfb3ecdb6a132e8da7569a6c442e6bb217";
      fetchSubmodules = false;
      sha256 = "sha256-YQ/2sveC57vQ36BjgnvPzObSAiAYmg1zK/TmEVqSvhY=";
    };
    date = "2026-07-06";
  };
  ferstar-gestures = {
    pname = "ferstar-gestures";
    version = "v0.8.3";
    src = fetchFromGitHub {
      owner = "ferstar";
      repo = "gestures";
      rev = "v0.8.3";
      fetchSubmodules = false;
      sha256 = "sha256-o0nPFTvvf+g95bi2Vf0kZMbgY8dWoLBgrNTrPExO6ro=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-o0nPFTvvf+g95bi2Vf0kZMbgY8dWoLBgrNTrPExO6ro=/Cargo.lock";
      outputHashes = {
        
      };
    };
  };
  gaze = {
    pname = "gaze";
    version = "8972873324c1ece91a51dfee3006794df4c28753";
    src = fetchFromGitHub {
      owner = "GunduLabs";
      repo = "gaze";
      rev = "8972873324c1ece91a51dfee3006794df4c28753";
      fetchSubmodules = false;
      sha256 = "sha256-jkA7tVo7milB+NanVbvwBNN1seKyN4YjGieu2pxRvug=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-jkA7tVo7milB+NanVbvwBNN1seKyN4YjGieu2pxRvug=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-08-15";
  };
  gaze-stable = {
    pname = "gaze-stable";
    version = "v0.2.10";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  geolite2-asn = {
    pname = "geolite2-asn";
    version = "2026.08.16";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2026.08.16/GeoLite2-ASN.mmdb";
      sha256 = "sha256-ElGBHM+L9JzdKY+xla1FbU+qG2qfsjucKW008OavS7I=";
    };
  };
  geolite2-city = {
    pname = "geolite2-city";
    version = "2026.08.16";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2026.08.16/GeoLite2-City.mmdb";
      sha256 = "sha256-Zz+0M/+j210XWuI134RtMq5ruUZ33npiXOUavKSJHag=";
    };
  };
  geolite2-country = {
    pname = "geolite2-country";
    version = "2026.08.16";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2026.08.16/GeoLite2-Country.mmdb";
      sha256 = "sha256-jMALvNlzTfgErMNhlshKvmXC70vrQpTCv00lrDVtuTM=";
    };
  };
  git-mcp-server = {
    pname = "git-mcp-server";
    version = "v2.15.1";
    src = fetchFromGitHub {
      owner = "cyanheads";
      repo = "git-mcp-server";
      rev = "v2.15.1";
      fetchSubmodules = false;
      sha256 = "sha256-CzKb4HRVrf/XyldYm69KJWn6cIpVAfz9Vg7q2j6SBdc=";
    };
  };
  happydeliver = {
    pname = "happydeliver";
    version = "v1.2.0";
    src = fetchFromGitHub {
      owner = "happyDomain";
      repo = "happydeliver";
      rev = "v1.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-VXkRDVHrRbU2TEl47jUhEmjXCkGzJgGYLKZBE/m9Ka8=";
    };
    extract = {
      "web/package.json" = ./. + "/sha256-VXkRDVHrRbU2TEl47jUhEmjXCkGzJgGYLKZBE_m9Ka8=/web/package.json";
      "web/package-lock.json" = ./. + "/sha256-VXkRDVHrRbU2TEl47jUhEmjXCkGzJgGYLKZBE_m9Ka8=/web/package-lock.json";
    };
  };
  headscale-ui = {
    pname = "headscale-ui";
    version = "ce7569c8473b70f6f8359385f2dbd6af66f21e55";
    src = fetchFromGitHub {
      owner = "gurucomputing";
      repo = "headscale-ui";
      rev = "ce7569c8473b70f6f8359385f2dbd6af66f21e55";
      fetchSubmodules = false;
      sha256 = "sha256-JR+VLwXqMKACWB+4AIgWLIpZh0xtU9uCsiskra71BHQ=";
    };
    extract = {
      "package-lock.json" = ./. + "/sha256-JR+VLwXqMKACWB+4AIgWLIpZh0xtU9uCsiskra71BHQ=/package-lock.json";
      "package.json" = ./. + "/sha256-JR+VLwXqMKACWB+4AIgWLIpZh0xtU9uCsiskra71BHQ=/package.json";
    };date = "2026-03-16";
  };
  hmcl-dev-bin = {
    pname = "hmcl-dev-bin";
    version = "3.17.0.354";
    src = fetchurl {
      url = "https://github.com/HMCL-dev/HMCL/releases/download/v3.17.0.354/HMCL-3.17.0.354.jar";
      sha256 = "sha256-TRRsfGmYIZgwUPWQ00jpReLYYubMcl4OmUzppHgt8Hg=";
    };
  };
  hmcl-dev-src = {
    pname = "hmcl-dev-src";
    version = "v3.17.0.354";
    src = fetchFromGitHub {
      owner = "HMCL-dev";
      repo = "HMCL";
      rev = "v3.17.0.354";
      fetchSubmodules = false;
      sha256 = "sha256-YLmFTgfTLGw9PKQu+o2zKP3/JFj47IGTDRdrXmuovBM=";
    };
  };
  hysteria-realm-server = {
    pname = "hysteria-realm-server";
    version = "v1.1.0";
    src = fetchFromGitHub {
      owner = "apernet";
      repo = "hysteria-realm-server";
      rev = "v1.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-Mz+pkJA+/mX1BwxXnAP41eitaJMzOFiNh0U9SCJx0iU=";
    };
  };
  libinput-three-finger-drag = {
    pname = "libinput-three-finger-drag";
    version = "6acd3f84b551b855b5f21b08db55e95dae3305c5";
    src = fetchFromGitHub {
      owner = "marsqing";
      repo = "libinput-three-finger-drag";
      rev = "6acd3f84b551b855b5f21b08db55e95dae3305c5";
      fetchSubmodules = false;
      sha256 = "sha256-xmcTb+23d6mMzIfMVjzN6bwV0fWH4p6YhXXqrFmt4TM=";
    };
    date = "2024-06-17";
  };
  loyalsoldier-geoip = {
    pname = "loyalsoldier-geoip";
    version = "c78da0e5e0dd25a2e929363205cd04881e737ccc";
    src = fetchFromGitHub {
      owner = "Loyalsoldier";
      repo = "geoip";
      rev = "c78da0e5e0dd25a2e929363205cd04881e737ccc";
      fetchSubmodules = false;
      sha256 = "sha256-xVRnTErBfE+uEt0HWvyp/Lj7p4NhVnTxbLgiFho0h7M=";
    };
    date = "2026-08-13";
  };
  manpage-zh = {
    pname = "manpage-zh";
    version = "v1.6.4.5";
    src = fetchFromGitHub {
      owner = "man-pages-zh";
      repo = "manpages-zh";
      rev = "v1.6.4.5";
      fetchSubmodules = true;
      sha256 = "sha256-UCWfefrPr5kFsN+HjpAA0IMbhXKOlb5WIpcbnw+2kf8=";
    };
  };
  mautrix-telegram = {
    pname = "mautrix-telegram";
    version = "v0.2607.0";
    src = fetchFromGitHub {
      owner = "mautrix";
      repo = "telegram";
      rev = "v0.2607.0";
      fetchSubmodules = false;
      sha256 = "sha256-MpdsWtEsVnC6purF5sw+RD+Nb/3Wo0xrzSn2BuFZmj8=";
    };
  };
  meta-rules-converter = {
    pname = "meta-rules-converter";
    version = "7dea27841a3579a633189830c98c08a0434e8b79";
    src = fetchFromGitHub {
      owner = "MetaCubeX";
      repo = "meta-rules-converter";
      rev = "7dea27841a3579a633189830c98c08a0434e8b79";
      fetchSubmodules = false;
      sha256 = "sha256-PwM5EH4v4MWePVeSjIGjCSu1ACYCIQ/wVq4sBTbrF9M=";
    };
    date = "2025-12-01";
  };
  meta-rules-dat = {
    pname = "meta-rules-dat";
    version = "9b775de81b488d5d2e03348797e6dd540aaabbcd";
    src = fetchFromGitHub {
      owner = "MetaCubeX";
      repo = "meta-rules-dat";
      rev = "9b775de81b488d5d2e03348797e6dd540aaabbcd";
      fetchSubmodules = false;
      sha256 = "sha256-pzHTH+Glr3ekLR+7zuXfp0VPgMTRbEBEXvNnMhQzRqc=";
    };
    date = "2026-08-15";
  };
  metacubex-geo = {
    pname = "metacubex-geo";
    version = "v1.1";
    src = fetchFromGitHub {
      owner = "metacubex";
      repo = "geo";
      rev = "v1.1";
      fetchSubmodules = false;
      sha256 = "sha256-lwFBevf3iP90LgnfUqweCjPBJPr2vMFtRqQXXUC+cRA=";
    };
  };
  microfetch = {
    pname = "microfetch";
    version = "d4c630c9f9564cc1666cdf60348cc15379767648";
    src = fetchFromGitHub {
      owner = "NotAShelf";
      repo = "microfetch";
      rev = "d4c630c9f9564cc1666cdf60348cc15379767648";
      fetchSubmodules = false;
      sha256 = "sha256-AMYfEHzLWw1l7Go3KlwTUmhMtqLT4QTsNCTz0AW88n8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-AMYfEHzLWw1l7Go3KlwTUmhMtqLT4QTsNCTz0AW88n8=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-08-10";
  };
  mm-geofeed-verifier = {
    pname = "mm-geofeed-verifier";
    version = "v4.0.0";
    src = fetchFromGitHub {
      owner = "maxmind";
      repo = "mm-geofeed-verifier";
      rev = "v4.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-QbILhdodWRV3J03YkV0uSYj0c7yaNHiD950US9JaE3s=";
    };
  };
  mpv-danmaku = {
    pname = "mpv-danmaku";
    version = "731dbc96c9b438b78b7ad115da181c6dc404982d";
    src = fetchFromGitHub {
      owner = "moraxyc";
      repo = "mpv-danmaku";
      rev = "731dbc96c9b438b78b7ad115da181c6dc404982d";
      fetchSubmodules = false;
      sha256 = "sha256-9Zp+MxUHDD/lEqwf8TNj/n+MKO4maW8TjH8HOkRqQIQ=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-9Zp+MxUHDD_lEqwf8TNj_n+MKO4maW8TjH8HOkRqQIQ=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-07-31";
  };
  mpv-danmaku-stable = {
    pname = "mpv-danmaku-stable";
    version = "v1.9.0";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  nezha = {
    pname = "nezha";
    version = "v2.3.6";
    src = fetchFromGitHub {
      owner = "nezhahq";
      repo = "nezha";
      rev = "v2.3.6";
      fetchSubmodules = false;
      sha256 = "sha256-xl5F6EaxhXogLRgB0voFijYaK69Pq6G54+VzoUBYmr0=";
    };
  };
  nezha-agent = {
    pname = "nezha-agent";
    version = "v2.3.3";
    src = fetchFromGitHub {
      owner = "nezhahq";
      repo = "agent";
      rev = "v2.3.3";
      fetchSubmodules = false;
      sha256 = "sha256-cSjioxBFbokyPwFhPhzpMUUIHAOaJtVsbyBrUiAt99Y=";
    };
  };
  nezha-theme-admin = {
    pname = "nezha-theme-admin";
    version = "v2.3.4";
    src = fetchFromGitHub {
      owner = "nezhahq";
      repo = "admin-frontend";
      rev = "v2.3.4";
      fetchSubmodules = false;
      sha256 = "sha256-cjq0dwIUsT4iNcatzwxOaMMJ9dpjNmSShjHBBXW6neA=";
    };
    extract = {
      "package-lock.json" = ./. + "/sha256-cjq0dwIUsT4iNcatzwxOaMMJ9dpjNmSShjHBBXW6neA=/package-lock.json";
      "package.json" = ./. + "/sha256-cjq0dwIUsT4iNcatzwxOaMMJ9dpjNmSShjHBBXW6neA=/package.json";
    };
  };
  nezha-theme-user = {
    pname = "nezha-theme-user";
    version = "v2.4.2";
    src = fetchFromGitHub {
      owner = "hamster1963";
      repo = "nezha-dash-v2";
      rev = "v2.4.2";
      fetchSubmodules = false;
      sha256 = "sha256-n1q9mjheW2J500mEPhBaGHuMAEVtddxXCyLC+Au/8Ts=";
    };
  };
  nipaplay-reload-darwin = {
    pname = "nipaplay-reload-darwin";
    version = "1.11.4";
    src = fetchurl {
      url = "https://github.com/MCDFsteve/NipaPlay-Reload/releases/download/v1.11.4/NipaPlay_1.11.4_macOS_Universal.dmg";
      sha256 = "sha256-dPDE+IJKHDPZqEoUmm0fuz+95rnm9NtALUSZO2F/ar0=";
    };
  };
  nipaplay-reload-x86_64-linux = {
    pname = "nipaplay-reload-x86_64-linux";
    version = "1.11.4";
    src = fetchurl {
      url = "https://github.com/MCDFsteve/NipaPlay-Reload/releases/download/v1.11.4/NipaPlay-1.11.4-Linux-amd64.AppImage";
      sha256 = "sha256-FLfJK8yUH7XwoPfkhsZpgVcF85fZRAy0ebG/0DfjtWc=";
    };
  };
  nyanpasu-service = {
    pname = "nyanpasu-service";
    version = "e899bce34200d0bc32bd789a79e7b53dc687c336";
    src = fetchFromGitHub {
      owner = "libnyanpasu";
      repo = "nyanpasu-service";
      rev = "e899bce34200d0bc32bd789a79e7b53dc687c336";
      fetchSubmodules = false;
      sha256 = "sha256-S1qni79VpcC66B3zxOLtsQIue7+uQZQpj6pwXhK0LhA=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-S1qni79VpcC66B3zxOLtsQIue7+uQZQpj6pwXhK0LhA=/Cargo.lock";
      outputHashes = {
        "processkit-2.2.5" = "sha256-aLH0vrplsy8uTBzm5ult//Ix4DGE/Qg5TQyCBlUCCJo=";
      };
    };
    date = "2026-08-12";
  };
  nyanpasu-service-stable = {
    pname = "nyanpasu-service-stable";
    version = "v1.4.5";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  opencode-telegram-bot = {
    pname = "opencode-telegram-bot";
    version = "v0.24.0";
    src = fetchFromGitHub {
      owner = "grinev";
      repo = "opencode-telegram-bot";
      rev = "v0.24.0";
      fetchSubmodules = false;
      sha256 = "sha256-uZaAyt0CQtB2GiG33+h0yDSFjuOoAYuW949coHQHVsQ=";
    };
    extract = {
      "package-lock.json" = ./. + "/sha256-uZaAyt0CQtB2GiG33+h0yDSFjuOoAYuW949coHQHVsQ=/package-lock.json";
      "package.json" = ./. + "/sha256-uZaAyt0CQtB2GiG33+h0yDSFjuOoAYuW949coHQHVsQ=/package.json";
    };
  };
  picoclaw = {
    pname = "picoclaw";
    version = "v0.3.1";
    src = fetchFromGitHub {
      owner = "sipeed";
      repo = "picoclaw";
      rev = "v0.3.1";
      fetchSubmodules = false;
      sha256 = "sha256-QYfHXIHJjeZJdkhGNNhdO91Q8EFgSP+ubqGJ8VgTEtc=";
    };
  };
  pixy2 = {
    pname = "pixy2";
    version = "d7c38d49ae8071b58e46f5b35eda927dd2b7c621";
    src = fetchFromGitHub {
      owner = "charmedlabs";
      repo = "pixy2";
      rev = "d7c38d49ae8071b58e46f5b35eda927dd2b7c621";
      fetchSubmodules = false;
      sha256 = "sha256-w0t7KqV8OS3PJBdZtLn86zknQYo70rBv7fuK/UFx9zQ=";
    };
    date = "2024-07-17";
  };
  shard = {
    pname = "shard";
    version = "1368a10a29ff948921ce8007de8ce8778170227a";
    src = fetchFromGitHub {
      owner = "Th0rgal";
      repo = "shard";
      rev = "1368a10a29ff948921ce8007de8ce8778170227a";
      fetchSubmodules = false;
      sha256 = "sha256-Mn8sR07ZRb+NFvd5sAPPnC/Pa2rWVDXgQKjGywVglNA=";
    };
    extract = {
      "desktop/package-lock.json" = ./. + "/sha256-Mn8sR07ZRb+NFvd5sAPPnC_Pa2rWVDXgQKjGywVglNA=/desktop/package-lock.json";
      "desktop/package.json" = ./. + "/sha256-Mn8sR07ZRb+NFvd5sAPPnC_Pa2rWVDXgQKjGywVglNA=/desktop/package.json";
    };cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-Mn8sR07ZRb+NFvd5sAPPnC_Pa2rWVDXgQKjGywVglNA=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-06-01";
  };
  shard-stable = {
    pname = "shard-stable";
    version = "v0.1.25";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
  shlink = {
    pname = "shlink";
    version = "v5.1.5";
    src = fetchFromGitHub {
      owner = "shlinkio";
      repo = "shlink";
      rev = "v5.1.5";
      fetchSubmodules = false;
      sha256 = "sha256-5Kz98zgWh8a/SDNLS+C1G+KBRrrPbYVYWv/lugJFGIs=";
    };
  };
  subconverter = {
    pname = "subconverter";
    version = "a0d4eab28cb8b6c782d4ce5c3a918de4829b4a72";
    src = fetchFromGitHub {
      owner = "tindy2013";
      repo = "subconverter";
      rev = "a0d4eab28cb8b6c782d4ce5c3a918de4829b4a72";
      fetchSubmodules = false;
      sha256 = "sha256-5GH2HEcloEc6XooJ0YclLF7XS/0KGOVnaK8Nsy0mSUg=";
    };
    date = "2026-07-08";
  };
  superpowers-skills = {
    pname = "superpowers-skills";
    version = "b36e0829c6d0140e93cfef2ca599b1b07d4a7797";
    src = fetchFromGitHub {
      owner = "obra";
      repo = "superpowers";
      rev = "b36e0829c6d0140e93cfef2ca599b1b07d4a7797";
      fetchSubmodules = false;
      sha256 = "sha256-EsGNO0dULWf5Bx6bGrCv2kI2Z8aKH0kRvGiuN23wChQ=";
    };
    date = "2026-08-12";
  };
  udpxy = {
    pname = "udpxy";
    version = "1.0-25.2";
    src = fetchFromGitHub {
      owner = "pcherenkov";
      repo = "udpxy";
      rev = "1.0-25.2";
      fetchSubmodules = false;
      sha256 = "sha256-v+w4Y6MyJqUrgwuYUYTZW0Zn1jhW4vEpgBEQyEjvkzg=";
    };
  };
  v2ray-rules-dat = {
    pname = "v2ray-rules-dat";
    version = "232049753662eface16308cf8c589a5666c8999e";
    src = fetchFromGitHub {
      owner = "Loyalsoldier";
      repo = "v2ray-rules-dat";
      rev = "232049753662eface16308cf8c589a5666c8999e";
      fetchSubmodules = false;
      sha256 = "sha256-vU7J7thRBoSV5g3d8tUoYRUZAuqwugNQf0BTZkZZwAs=";
    };
    date = "2026-08-15";
  };
  whois42d-ng = {
    pname = "whois42d-ng";
    version = "460924c8ea74d3e35c9835699d447da2bbed972e";
    src = fetchFromGitHub {
      owner = "moraxyc";
      repo = "whois42d-ng";
      rev = "460924c8ea74d3e35c9835699d447da2bbed972e";
      fetchSubmodules = false;
      sha256 = "sha256-gwaf809LvUv6Lw0h/KoLvZuSZzA5nf9WvgLLTysRZKI=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-gwaf809LvUv6Lw0h_KoLvZuSZzA5nf9WvgLLTysRZKI=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-08-13";
  };
  whois42d-ng-stable = {
    pname = "whois42d-ng-stable";
    version = "v2.1.0";
    src = fetchurl {
      url = "file:///dev/null";
      sha256 = "sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=";
    };
  };
}
