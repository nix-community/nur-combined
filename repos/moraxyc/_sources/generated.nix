# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  aliyundrive-webdav = {
    pname = "aliyundrive-webdav";
    version = "6e8eba62b4e50acf89681e3a67a3a693186dcd05";
    src = fetchFromGitHub {
      owner = "messense";
      repo = "aliyundrive-webdav";
      rev = "6e8eba62b4e50acf89681e3a67a3a693186dcd05";
      fetchSubmodules = false;
      sha256 = "sha256-jyAq4B7SNbw++eoUFoI/Rc9GucU/1NMrC1/Rjysr5wg=";
    };
    date = "2025-06-10";
  };
  aliyundrive-webdav-stable = {
    pname = "aliyundrive-webdav-stable";
    version = "v2.3.3";
    src = fetchFromGitHub {
      owner = "messense";
      repo = "aliyundrive-webdav";
      rev = "v2.3.3";
      fetchSubmodules = false;
      sha256 = "sha256-LN2HdmhcPk6RCrRZdV3A8InoYgqJSHC2XjjuKZLfVaI=";
    };
  };
  bark-server = {
    pname = "bark-server";
    version = "9e50f46b7cfd231b699bd76fe32b759048871aa0";
    src = fetchFromGitHub {
      owner = "Finb";
      repo = "bark-server";
      rev = "9e50f46b7cfd231b699bd76fe32b759048871aa0";
      fetchSubmodules = false;
      sha256 = "sha256-u2BN4HJDQdhpJJATH0fD4c4WhSCMngEq4iw1IoUcAd4=";
    };
    date = "2026-02-04";
  };
  bark-server-stable = {
    pname = "bark-server-stable";
    version = "v2.3.3";
    src = fetchFromGitHub {
      owner = "Finb";
      repo = "bark-server";
      rev = "v2.3.3";
      fetchSubmodules = false;
      sha256 = "sha256-Pnj4ZRzW0ZW1mgtl+wT7JE/ccJ9kivGBe6tgbBiBMLs=";
    };
  };
  boringssl-oqs = {
    pname = "boringssl-oqs";
    version = "OQS-BoringSSL-snapshot-2025-01";
    src = fetchFromGitHub {
      owner = "open-quantum-safe";
      repo = "boringssl";
      rev = "OQS-BoringSSL-snapshot-2025-01";
      fetchSubmodules = false;
      sha256 = "sha256-zVZgAvq6V85hxO79Ct8s+VVyf9yPa/YbRL4D5z31qEs=";
    };
  };
  english_words = {
    pname = "english_words";
    version = "20f5cc9b3f0ccc8ce45d814c532b7c2031bba31c";
    src = fetchFromGitHub {
      owner = "dwyl";
      repo = "english-words";
      rev = "20f5cc9b3f0ccc8ce45d814c532b7c2031bba31c";
      fetchSubmodules = false;
      sha256 = "sha256-PfBQ9iavOLx7M8+j0TXASUxVamPiQn2YXsiuUmz4XCg=";
    };
    date = "2025-01-06";
  };
  exloli-next = {
    pname = "exloli-next";
    version = "41d7a6e52a1625d6febb47160a4b143ca47c16d5";
    src = fetchFromGitHub {
      owner = "lolishinshi";
      repo = "exloli-next";
      rev = "41d7a6e52a1625d6febb47160a4b143ca47c16d5";
      fetchSubmodules = false;
      sha256 = "sha256-S3hjuRSDtPkK9XF2USXfyOawWoD/fu9qYDJbUPI3Op0=";
    };
    date = "2024-11-23";
  };
  fcitx5-themes-candlelight = {
    pname = "fcitx5-themes-candlelight";
    version = "653677b0454569f41c815b3d262a57e42c90ee05";
    src = fetchFromGitHub {
      owner = "thep0y";
      repo = "fcitx5-themes-candlelight";
      rev = "653677b0454569f41c815b3d262a57e42c90ee05";
      fetchSubmodules = false;
      sha256 = "sha256-dN77aUt1qkN177BZOfrT6O72Qp0J2jlM2mGNxI0cBnA=";
    };
    date = "2025-11-07";
  };
  flapalerted = {
    pname = "flapalerted";
    version = "ce2cbb87fa73c5193876cbe5949aa22bbd0a8fa8";
    src = fetchFromGitHub {
      owner = "Kioubit";
      repo = "FlapAlerted";
      rev = "ce2cbb87fa73c5193876cbe5949aa22bbd0a8fa8";
      fetchSubmodules = false;
      sha256 = "sha256-gsX1RVJ8HP4cjkXVYRttwAewfElwtK6FpGW60x3iCos=";
    };
    date = "2026-01-31";
  };
  flapalerted-stable = {
    pname = "flapalerted-stable";
    version = "v4.3.0";
    src = fetchFromGitHub {
      owner = "Kioubit";
      repo = "FlapAlerted";
      rev = "v4.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-xoskkM5xdU3+T5AtgGv0uPZEUOLtMGl7RPv8gKBJBho=";
    };
  };
  geolite2-asn = {
    pname = "geolite2-asn";
    version = "2026.02.16";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2026.02.16/GeoLite2-ASN.mmdb";
      sha256 = "sha256-QbxiMYElwoUfUqKDDyBtGPsFVUqkmvme7vftZHJlnCY=";
    };
  };
  geolite2-city = {
    pname = "geolite2-city";
    version = "2026.02.16";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2026.02.16/GeoLite2-City.mmdb";
      sha256 = "sha256-focZgNDBjgFRS5SeA0/rAe6azRMLdUMdf7kP1CedYqc=";
    };
  };
  geolite2-country = {
    pname = "geolite2-country";
    version = "2026.02.16";
    src = fetchurl {
      url = "https://github.com/P3TERX/GeoLite.mmdb/releases/download/2026.02.16/GeoLite2-Country.mmdb";
      sha256 = "sha256-mFtvdNbT/b/YRjRZiAXtq5UiAlVrnN9JaqfnK8HZnXs=";
    };
  };
  git-mcp-server = {
    pname = "git-mcp-server";
    version = "v2.8.4";
    src = fetchFromGitHub {
      owner = "cyanheads";
      repo = "git-mcp-server";
      rev = "v2.8.4";
      fetchSubmodules = false;
      sha256 = "sha256-bB0EHBvhFOk565wGNLvhJfBLWk/25S/D2b7mW23vp44=";
    };
  };
  happydeliver = {
    pname = "happydeliver";
    version = "v1.1.1";
    src = fetchFromGitHub {
      owner = "happyDomain";
      repo = "happydeliver";
      rev = "v1.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-+1eJUqCAjae5syxSEvcn3vdgYeV5kSt6ckbDDwK7bB0=";
    };
    extract = {
      "web/package.json" = ./. + "/sha256-+1eJUqCAjae5syxSEvcn3vdgYeV5kSt6ckbDDwK7bB0=/web/package.json";
      "web/package-lock.json" = ./. + "/sha256-+1eJUqCAjae5syxSEvcn3vdgYeV5kSt6ckbDDwK7bB0=/web/package-lock.json";
    };
  };
  libinput-three-finger-drag = {
    pname = "libinput-three-finger-drag";
    version = "6acd3f84b551b855b5f21b08db55e95dae3305c5";
    src = fetchFromGitHub {
      owner = "marsqing";
      repo = "libinput-three-finger-drag";
      rev = "6acd3f84b551b855b5f21b08db55e95dae3305c5";
      fetchSubmodules = false;
      sha256 = "sha256-xmcTb+23d6mMzIfMVjzN6bwV0fWH4p6YhXXqrFmt4TM=";
    };
    date = "2024-06-17";
  };
  mautrix-telegramgo = {
    pname = "mautrix-telegramgo";
    version = "52b237352837c4ddaa4d2cecd3bd84c37dedeaf0";
    src = fetchFromGitHub {
      owner = "mautrix";
      repo = "telegramgo";
      rev = "52b237352837c4ddaa4d2cecd3bd84c37dedeaf0";
      fetchSubmodules = false;
      sha256 = "sha256-4tlidsxK3MCc42vlK2BPrGjlroCXE1Ojs89bbNwr4Ms=";
    };
    date = "2026-02-16";
  };
  mm-geofeed-verifier = {
    pname = "mm-geofeed-verifier";
    version = "v4.0.0";
    src = fetchFromGitHub {
      owner = "maxmind";
      repo = "mm-geofeed-verifier";
      rev = "v4.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-QbILhdodWRV3J03YkV0uSYj0c7yaNHiD950US9JaE3s=";
    };
  };
  mpv-danmaku = {
    pname = "mpv-danmaku";
    version = "4e4509dbb577a07e17acdecf2abd56f646a8dd8d";
    src = fetchFromGitHub {
      owner = "moraxyc";
      repo = "mpv-danmaku";
      rev = "4e4509dbb577a07e17acdecf2abd56f646a8dd8d";
      fetchSubmodules = false;
      sha256 = "sha256-xmK7SL7k4xBhkAvUvZbUsX1ZjXDmaA1QnvuR9eFh2z8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-xmK7SL7k4xBhkAvUvZbUsX1ZjXDmaA1QnvuR9eFh2z8=/Cargo.lock";
      outputHashes = {
        
      };
    };
    date = "2026-02-17";
  };
  mpv-danmaku-stable = {
    pname = "mpv-danmaku-stable";
    version = "v1.9.0";
    src = fetchFromGitHub {
      owner = "moraxyc";
      repo = "mpv-danmaku";
      rev = "v1.9.0";
      fetchSubmodules = false;
      sha256 = "sha256-N+Vi+mbKGogO6PgONqiegbrHGh+5BePePkU9+Irate0=";
    };
  };
  nipaplay-reload-darwin = {
    pname = "nipaplay-reload-darwin";
    version = "1.9.11";
    src = fetchurl {
      url = "https://github.com/MCDFsteve/NipaPlay-Reload/releases/download/v1.9.11/NipaPlay_1.9.11_macOS_Universal.dmg";
      sha256 = "sha256-YNjl9nXG+S98Pkhw5XUgPhz0ncOetVRgmJuiv/qRCtE=";
    };
  };
  nipaplay-reload-x86_64-linux = {
    pname = "nipaplay-reload-x86_64-linux";
    version = "1.9.11";
    src = fetchurl {
      url = "https://github.com/MCDFsteve/NipaPlay-Reload/releases/download/v1.9.11/NipaPlay-1.9.11-Linux-amd64.AppImage";
      sha256 = "sha256-ROQ9bApGi57buIp6cQjbSFA2UW/eliA8qzveA3fxVjI=";
    };
  };
  niri = {
    pname = "niri";
    version = "/nix/store/ksxp4pm8sz3jd9l7z4v0v7aar8yk6dmd-niri-patched-src.tar.gz";
    src = fetchurl {
      url = "file:///nix/store/ksxp4pm8sz3jd9l7z4v0v7aar8yk6dmd-niri-patched-src.tar.gz";
      sha256 = "sha256-pe3FH8ILUjFTVH8uRS8oVNrM72o3gs9KBL3FpDo2xu4=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./. + "/sha256-pe3FH8ILUjFTVH8uRS8oVNrM72o3gs9KBL3FpDo2xu4=/Cargo.lock";
      outputHashes = {
        "smithay-0.7.0" = "sha256-By+gqymYHqlrcLzy6J90i2utsxsmr1SP17jodA8apig=";
        "reis-0.5.0" = "sha256-e6F52FCV5sHYXjBwLvpUg/EGfUjtUfVto3PVedbti28=";
      };
    };
  };
  pixy2 = {
    pname = "pixy2";
    version = "d7c38d49ae8071b58e46f5b35eda927dd2b7c621";
    src = fetchFromGitHub {
      owner = "charmedlabs";
      repo = "pixy2";
      rev = "d7c38d49ae8071b58e46f5b35eda927dd2b7c621";
      fetchSubmodules = false;
      sha256 = "sha256-w0t7KqV8OS3PJBdZtLn86zknQYo70rBv7fuK/UFx9zQ=";
    };
    date = "2024-07-17";
  };
  v2ray-rules-dat-geoip = {
    pname = "v2ray-rules-dat-geoip";
    version = "202602172224";
    src = fetchurl {
      url = "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/202602172224/geoip.dat";
      sha256 = "sha256-LU+b02onQ+VdfFdPB1E0RKtwb1bMmU6fN9J75PcxOHo=";
    };
  };
  v2ray-rules-dat-geosite = {
    pname = "v2ray-rules-dat-geosite";
    version = "202602172224";
    src = fetchurl {
      url = "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/202602172224/geosite.dat";
      sha256 = "sha256-ZYJ7gkcrpEpGxbKBMHoIoeQ+NSjARJ1a3CjEelKOwws=";
    };
  };
}
