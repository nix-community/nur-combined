...
mycurl() {
  curl -f --verbose --referer $URL -H "Accept: application/json" -H "Content-Type: application/json" $@
}
...
EOF
mycurl -X PUT $URL/project/$PROJECT_NAME -d @data.json -b hydra-cookie.txt
...
EOF
mycurl -X PUT $URL/jobset/$PROJECT_NAME/$JOBSET_NAME -d @data.json -b hydra-cookie.txt
