    ...
    text = ''
      ...
      for file in pkgs/*/default.nix; do
        ...
        if [[ "''${#forced[@]}" -gt 0 ]]; then
          ...
          # A Cargo.nix without crate2nix's @generated header is a
          # committed placeholder (new packages are added with one so the
          # package set evaluates before the first update run) and must
          # always be regenerated, regardless of source-change detection.
