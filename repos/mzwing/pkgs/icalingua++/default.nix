...
in
  stdenv.mkDerivation {
    ...
    preBuild = ''
      # pnpm skips install scripts, so run the same command better-sqlite3's own build-release script does.
      # Without --force_build its binding.gyp turns both targets into `type: none` whenever a vendored prebuild matches the host.
