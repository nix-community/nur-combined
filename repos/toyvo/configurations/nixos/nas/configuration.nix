...
in
{
  ...
  services = {
    ...
    postgresql = {
      ...
      authentication = lib.mkOverride 10 ''
        ...
        host  authentik  authentik  10.1.0.0/16  scram-sha-256
      '';
    };
  ...
  # to exchange. Render the .runner credentials file directly instead; the
  # daemon reads it from its working directory and skips registration.
  # uuid: from the runner created in the Forgejo UI (replace if recreated)
  # token: the runner's secret, via the forgejo-runner-token sops secret
  systemd.services.gitea-runner-nas.serviceConfig.ExecStartPre = lib.mkForce (
    let
      runner = config.services.gitea-actions-runner.instances.nas;
    in
    pkgs.writeShellScript "forgejo-runner-nas-credentials" ''
      set -eu
      INSTANCE_DIR="$STATE_DIRECTORY/nas"
      mkdir -p "$INSTANCE_DIR"
      jq -n \
        --arg uuid "$UUID" \
        --arg name "${runner.name}" \
        --arg token "$TOKEN" \
        --arg address "${runner.url}" \
        --argjson labels '${builtins.toJSON runner.labels}' \
        '{
  ...
  # The runner's secret from the pre-created runner in the Forgejo UI (Site
  # Administration -> Actions -> Runners -> Create new runner). The template
  # wraps it into the TOKEN= env file used when rendering .runner credentials.
  sops.secrets."forgejo-runner-token-nas" = { };
  sops.secrets."forgejo-runner-uuid-nas" = { };
  sops.templates."forgejo-runner-token-nas.env".content = ''
    TOKEN=${config.sops.placeholder.forgejo-runner-token-nas}
    UUID=${config.sops.placeholder.forgejo-runner-uuid-nas}
  '';
