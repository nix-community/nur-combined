{
  ...
  hostCpuTargets ? (
    ...
    else if userOnly then
      map (system: (lib.systems.elaborate { inherit system; }).qemuArch + "-linux-user") [
        "aarch64-linux"
        "armv7l-linux"
        "i386-linux"
        "x86_64-linux"
...
stdenv.mkDerivation (finalAttrs: {
  ...
  meta =
    ...
    // lib.optionalAttrs (!toolsOnly && !userOnly) { mainProgram = "qemu-kvm"; }
    # userOnly: Only meaningful on linux
    // lib.optionalAttrs userOnly {
      platforms = lib.platforms.linux;
      description = "Variant of qemu building user space emulator only, from https://github.com/NixOS/nixpkgs/pull/314998";
    };
})
