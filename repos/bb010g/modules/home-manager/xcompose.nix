...
let
  ...
  includeRuleType = ty.submodule ({ config, lib, ... }: {
    ...
    options.literalInclude = lib.mkOption {
      ...
      default = null;
      example = lib.literalExample
        ''"/home/alice/literal%include.Compose"'';
    };

    config = lib.mkIf (config.literalInclude != null) {
      include = if lib.isString config.literalInclude
        then lib.replaceStrings ["%"] ["%%"] config.literalInclude
        else config.literalInclude;
    };
  });
  ...
  renderCommentLines = lines: let
    lines' = builtins.split "\n" lines;
    lineCount = lib.length lines' / 2 + 1;
    lines'' = builtins.genList
      (i: let line = lib.elemAt lines' (i * 2); in
        if line == "" then "#" else "# ${line}")
      lineCount;
  in concatNewline lines'';

  renderSequenceRule = rule: let
  ...
  renderRule = rule:
    (if rule ? commentLines then renderCommentRule
    else if rule ? include then renderIncludeRule
    else renderSequenceRule) rule;

  renderRules = rules:
    concatNewline (builtins.map renderRule rules ++ [ "" ]);
  ...
  options = {
    xcompose = {
      ...
      rules = lib.mkOption {
        ...
        '';
        description = "User compose rules.";
      };
    };
  };

  config = lib.mkIf cfg.enable {
    home.file.".XCompose".text = renderRules cfg.rules;
  };
}
