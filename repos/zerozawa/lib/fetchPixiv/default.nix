...
assert builtins.isList extensions && extensions != [ ] && builtins.all isNonEmptyString extensions;

fetchurl {
  inherit urls;
  inherit sha256;
  name = "pixiv-${toString id}${lib.optionalString (p > 0) "-p${toString p}"}";
}
