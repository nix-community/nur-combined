...
- `JMComic-qt` 1.3.4.1 still commits `src/images_rc.py`, so copying `src/` is sufficient; do not assume it shares PicACG's resource-generation change. Its upstream code already handles XDG paths. Keep `smbprotocol` available: upload connection checks import the SMBv3 backend even for local-directory targets, so a GUI-only smoke test misses this dependency
- `picacg-qt` 1.5.5 no longer commits `src/images_rc.py`; generate it from `res/images.qrc` with Qt's `rcc -g python` before installation. Its runtime now requires `curl-cffi` and `smbprotocol` instead of `httpx`, and the launcher uses `Setting.GetDBPath()` to seed the database in the application's XDG data directory. Keep the installed-resource image check enabled via `doCheck` (the Python builder derives `doInstallCheck` from it)
