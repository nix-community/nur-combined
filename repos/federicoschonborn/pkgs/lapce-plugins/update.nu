...
def main [
	...
	let upstream_plugins = http get $"https://plugins.lapce.dev/api/v1/plugins/?limit=($plugin_count)" | get plugins | select ...[author name version description wasm] | sort-by author name
	...
	$upstream_plugins | group-by author | items { |$author, $plugins|
		...
		for plugin in $plugins {
			try {
				...
				$collected = $collected | append $'"($clean_name)" = mkLapcePlugin { author = "($author)"; name = "($plugin.name)"; version = "($plugin.version)"; hash = "($sri_hash)"; wasm = ($plugin.wasm); };'
			} catch { |err|
				print --stderr $err.msg
				return
			}
		}
		$'"($clean_author)" = lib.recurseIntoAttrs {' | save --append $output
