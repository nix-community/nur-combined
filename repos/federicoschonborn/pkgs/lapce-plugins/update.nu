...
def main [] {
	...
	let plugins = http get $"https://plugins.lapce.dev/api/v1/plugins/?limit=($plugin_count)"
		| get plugins
	...
	for plugin in $plugins {
		...
		let sri_hash = if not ($store_file_name in $old_hashes) {
			let download_url = http get --raw $"https://plugins.lapce.dev/api/v1/plugins/($plugin.author)/($plugin.name)/($plugin.version)/download"
			let sha256_hash = try {
