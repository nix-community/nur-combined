# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  claudecode-nvim = {
    pname = "claudecode-nvim";
    version = "2390c6e45c4789072c293ac69de051d169668b29";
    src = fetchFromGitHub {
      owner = "coder";
      repo = "claudecode.nvim";
      rev = "2390c6e45c4789072c293ac69de051d169668b29";
      fetchSubmodules = false;
      sha256 = "sha256-oMBPSRQFDmJ9Lq+ZP8vFMHaocm4sPX3D/orVMNwVXuM=";
    };
    date = "2026-06-25";
  };
  copyq-darwin = {
    pname = "copyq-darwin";
    version = "16.0.0";
    src = fetchurl {
      url = "https://github.com/hluk/CopyQ/releases/download/v16.0.0/CopyQ-16.0.0-macos-12-m1.dmg";
      sha256 = "sha256-V1Y/ssokdRl0w1t0TKfqLFwXG8XAD1mzyDeZEodq5LE=";
    };
  };
  emacs-plus = {
    pname = "emacs-plus";
    version = "4c36f7c47b8556fc1d00571dd572f77e24f7b1ad";
    src = fetchFromGitHub {
      owner = "d12frosted";
      repo = "homebrew-emacs-plus";
      rev = "4c36f7c47b8556fc1d00571dd572f77e24f7b1ad";
      fetchSubmodules = false;
      sha256 = "sha256-QsSndeFrozpZdQWOML6A+mPrOsXjgJWpxmTt3aZsnRs=";
    };
    date = "2026-07-01";
  };
  hammerspoon = {
    pname = "hammerspoon";
    version = "1.1.1";
    src = fetchurl {
      url = "https://github.com/Hammerspoon/hammerspoon/releases/download/1.1.1/Hammerspoon-1.1.1.zip";
      sha256 = "sha256-EbsckPr1Qn83x71P5+q5d0rkPh1csCDFswiNrDKEnvo=";
    };
  };
  paperwm-spoon = {
    pname = "paperwm-spoon";
    version = "3a6d076c5169d693e6271d5af5038036bd591bbd";
    src = fetchFromGitHub {
      owner = "mogenson";
      repo = "PaperWM.spoon";
      rev = "3a6d076c5169d693e6271d5af5038036bd591bbd";
      fetchSubmodules = false;
      sha256 = "sha256-H9A7nwt0NkyPqN+iOZ9BU24dkAvvfZxucVSPhNx9WPA=";
    };
    date = "2026-05-29";
  };
  qmk-toolbox = {
    pname = "qmk-toolbox";
    version = "0.3.3";
    src = fetchurl {
      url = "https://github.com/qmk/qmk_toolbox/releases/download/0.3.3/QMK.Toolbox.app.zip";
      sha256 = "sha256-WPre2csGAQzavtksLbj3L/MrWUT6d2gTJVq7eAmpcLk=";
    };
  };
  qutebrowser-darwin = {
    pname = "qutebrowser-darwin";
    version = "3.7.0";
    src = fetchurl {
      url = "https://github.com/qutebrowser/qutebrowser/releases/download/v3.7.0/qutebrowser-3.7.0-arm64.dmg";
      sha256 = "sha256-mBcCQb8Sov4d6r/8gSC3DcjgDlVbtxLDh1HESPBJVKM=";
    };
  };
  sbarlua = {
    pname = "sbarlua";
    version = "dba9cc421b868c918d5c23c408544a28aadf2f2f";
    src = fetchFromGitHub {
      owner = "FelixKratz";
      repo = "SbarLua";
      rev = "dba9cc421b868c918d5c23c408544a28aadf2f2f";
      fetchSubmodules = false;
      sha256 = "sha256-lhLTrdufA3ALJ2S5HLdgNOr5seWIWEHkVhZNPObzbvI=";
    };
    date = "2026-03-06";
  };
  skills-ref = {
    pname = "skills-ref";
    version = "6c9ed64a4ba0b46502686c675f32b1c44f0ebc3e";
    src = fetchFromGitHub {
      owner = "agentskills";
      repo = "agentskills";
      rev = "6c9ed64a4ba0b46502686c675f32b1c44f0ebc3e";
      fetchSubmodules = false;
      sha256 = "sha256-SzPtMrt6qVmorJXIAkjYpgTdf6bHSMRQ26OTjAsirNo=";
    };
    date = "2026-06-30";
  };
  skkeleton = {
    pname = "skkeleton";
    version = "770b265dd115dba96ea5314642e35764654ef08f";
    src = fetchFromGitHub {
      owner = "vim-skk";
      repo = "skkeleton";
      rev = "770b265dd115dba96ea5314642e35764654ef08f";
      fetchSubmodules = false;
      sha256 = "sha256-Gsl/LqUY8szRqYLnDVyDOIDCw+cisoKsS5Fl1zQ+unM=";
    };
    date = "2026-05-05";
  };
  vivaldi-darwin = {
    pname = "vivaldi-darwin";
    version = "8.0.4033.54";
    src = fetchurl {
      url = "https://downloads.vivaldi.com/stable/Vivaldi.8.0.4033.54.universal.dmg";
      sha256 = "sha256-PPZ5i8K08Gy5U3j9+eeuufJJeCs+MCrpmsXsaiFCXvk=";
    };
  };
  zen-browser = {
    pname = "zen-browser";
    version = "1.21.4b";
    src = fetchurl {
      url = "https://github.com/zen-browser/desktop/releases/download/1.21.4b/zen.macos-universal.dmg";
      sha256 = "sha256-QKcId26PJRhZpPpz7Uhfhi614bl741a/4BS0FFBP20o=";
    };
  };
}
