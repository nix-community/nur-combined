...
description: "OSS release prep: fork, sanitize, and package private/internal projects for safe open-source release. Chains 3 agents (forker, sanitizer, packager). Triggers: '/oss-release-prep', 'open source this', 'make this public', 'prepare OSS release'."
---

# OSS Release Prep Skill
...
- User wants to prepare a private repo for OSS release
...
| Command | Action |
|---------|--------|
| `/oss-release-prep fork PROJECT` | Full pipeline: fork + sanitize + package |
| `/oss-release-prep verify PROJECT` | Run sanitizer on existing repo |
| `/oss-release-prep package PROJECT` | Generate CLAUDE.md + setup.sh + README |
| `/oss-release-prep list` | Show all staged projects |
| `/oss-release-prep status PROJECT` | Show reports for a staged project |

## Protocol

### /oss-release-prep fork PROJECT
...
STAGING_PATH="$HOME/oss-release-staging/${PROJECT_NAME}"
```
...
```bash
mkdir -p $HOME/oss-release-staging/
```

#### Step 3: Run Forker Agent

Spawn the `oss-release-forker` agent:

```
Agent(
  description="Fork {PROJECT} for open-source",
  subagent_type="oss-release-forker",
  prompt="""
...
Spawn the `oss-release-sanitizer` agent:

```
Agent(
  description="Verify {PROJECT} sanitization",
  subagent_type="oss-release-sanitizer",
  prompt="""
...
Spawn the `oss-release-packager` agent:

```
Agent(
  description="Package {PROJECT} for open-source",
  subagent_type="oss-release-packager",
  prompt="""
...
Present to user:
```
OSS Release Prep Ready: {PROJECT_NAME}
...
### /oss-release-prep verify PROJECT

Run sanitizer independently. Resolve path: if PROJECT contains `/`, treat as a path. Otherwise check `$HOME/oss-release-staging/PROJECT`, then `$HOME/PROJECT`, then current directory.
...
### /oss-release-prep package PROJECT
...
### /oss-release-prep list

```bash
ls -d $HOME/oss-release-staging/*/
```
...
### /oss-release-prep status PROJECT

```bash
cat $HOME/oss-release-staging/${PROJECT}/SANITIZATION_REPORT.md
cat $HOME/oss-release-staging/${PROJECT}/FORK_REPORT.md
```

## Staging Layout

```
$HOME/oss-release-staging/
  my-project/
