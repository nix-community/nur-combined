...
1. Inspect the branch and target commit:
   ```bash
   git fetch --prune
   git status --short
   ...
   ```
2. Use `git commit --fixup <sha>` only when:
   ...
   - no remote branch contains `<sha>`.
3. Stage only the files or hunks for that fixup. Never mix unrelated changes into one fixup.
4. Use a normal semantic commit when no single target commit owns the change, the change adds
