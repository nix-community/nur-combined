...
, ...
  ...
  installPhase = ''
    cp -a res/fonts/${dir} "$out"
  '';

  meta = {
    homepage = "https://github.com/mathoudebine/turing-smart-screen-python/tree/${version.rev}/res/themes/${dir}";
    description = "This is \"${dir}\" font package for turing-smart-screen-python package." + (if dir != pname then " Notice that now it's renamed to \"${pname}\" so you need to use this name in your packages." else "");
