# Generated by ./update.sh - do not update manually!
# Last updated: 2026-02-09
{ fetchurl }:
let
  any-darwin = {
    version = "6.9.89-2026-02-09";
    src = fetchurl {
      url = "https://dldir1v6.qq.com/qqfile/qq/QQNT/Mac/QQ_6.9.89_260205_01.dmg";
      hash = "sha256-GXsZgj0hWNkR654G5GJ5eY0LqbrItjxn0pgdYke9Kak=";
    };
  };
in
{
  aarch64-darwin = any-darwin;
  x86_64-darwin = any-darwin;

  aarch64-linux = {
    version = "3.2.23-2026-01-22";
    src = fetchurl {
      url = "
https://dldir1v6.qq.com/qqfile/qq/QQNT/Linux/QQ_3.2.23_260108_arm64_01.deb";
      hash = "sha256-hw7TwOQX6bs6AhwdVlGLwvu6gtHD4YcThPZKt+IZI/c=";
    };
  };

  x86_64-linux = {
    version = "3.2.23-2026-01-22";
    src = fetchurl {
      url = "
https://dldir1v6.qq.com/qqfile/qq/QQNT/Linux/QQ_3.2.23_260108_amd64_01.deb";
      hash = "sha256-pCUnGcG+uK3ODaCev8MQzlDHnqVI9czkKVBXZdC/uoQ=";
    };
  };
}
