...
let
  makeBunpenArgv = callPackage ./makeBunpenArgv.nix { };
      ...
      checkSandboxed = runCommand "${sandboxedNonBin.name}-check-sandboxed"
        {
          ...
          passthru.flat = linkFarm "${sandboxedNonBin.name}-check-sandboxed-flat" (
            lib.getAttrs outputs checkSandboxed
          );
        }
  ...
  let
    extraSandboxerArgs' = makeBunpenArgv (
