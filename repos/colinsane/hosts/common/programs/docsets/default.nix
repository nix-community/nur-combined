...
in {
  sane.programs.docsets = {
    ...
    packageUnwrapped = pkgs.symlinkJoin {
      ...
      paths = map (pkg: "${toString pkg}/share/docsets") cfg.config.pkgs;
      postBuild = ''
  ...
  sane.programs.docsets.config.pkgs = [
    # packages which ship docsets natively:
    pkgs.docsets.lua-std
    # docsets.gtk
    (pkgs.docsets.nix-builtins.override { nix = config.sane.programs.nix.packageUnwrapped; })
    pkgs.docsets.nixpkgs-lib
    pkgs.docsets.python3-std
    pkgs.docsets.rust-std
  ] ++ lib.map
