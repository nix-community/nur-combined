...
in
{
  ...
  config = lib.mkIf (cfg.enable && config.sane.maxBuildCost >= 2) {
    ...
    #   version = lib.warnIf (lib.versionOlder "8656" prevAttrs.version) "llama-cpp is updated upstream: remove version override?" "8656";
    #   src = prevAttrs.src.overrideAttrs {
    #     hash = "sha256-suc9z8nMrQgQKhXGcMZyKl7rJPpbZFx/W/djcN9eI+0=";
    #   };
    #   postPatch = "";
    # });
